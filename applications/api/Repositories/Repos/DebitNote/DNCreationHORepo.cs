...
{
    public class DNCreationHORepo : IDNCreationHORepo
    {
        readonly IConfiguration __config;
        public DNCreationHORepo(IConfiguration config)
        {
            __config = config;
        }
        ...
        public async Task<IList<DNApprovedPromoList>> GetApprovedPromoforDN(GetApprovedPromoforDNBody body)
        {
            try
            {
                using IDbConnection conn = Connection;
                ...
                __param.Add("@channel", body.channel);
                __param.Add("@account", body.account);
                __param.Add("@userid", body.userid);
        ...
        public async Task<DNGetById> GetDNHO(int id)
        {
            try
            {
        ...
        public async Task<IList<DNReportDto>> GetDNReport(string year, int entity, int distributor, int channel, int account, string userid)
        {
            try
            {
                using IDbConnection conn = Connection;
                ...
                var __res = await conn.QueryAsync<DNReportDto>("ip_debetnote_report", __param, commandType: CommandType.StoredProcedure, commandTimeout: 180);
                return __res.ToList();
            }
            catch (System.Exception __ex)
            {
        ...
        public async Task<DNCreationGetWHTType> GetDNCreationHOGetWHTType(int promoId)
        {
            try
            {
                using IDbConnection conn = Connection;
