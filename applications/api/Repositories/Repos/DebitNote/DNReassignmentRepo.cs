...
{
    public class DNReassignmentRepo : IDNReassignmentRepo
    {
        readonly IConfiguration __config;
        public DNReassignmentRepo(IConfiguration config)
        {
            __config = config;
        }
        ...
        public async Task<IList<ApprovalPromoforDNbyInitiator>> GetApprovedPromoByInitiator(string periode, int entityId, int channelId, int accountId, string userid)
        {
            try
            {
                using IDbConnection conn = Connection;
                ...
                var result = await conn.QueryAsync<ApprovalPromoforDNbyInitiator>("ip_promo_approved_list_for_dn_by_initiator", __param, commandType: CommandType.StoredProcedure, commandTimeout:180);
        ...
        public async Task<object> GetDNAssignList(string periode, int entityId, int distributorId, string channelId, string accountId, string userid, bool isdnmanual,
            string sortColumn, string sortDirection, int length = 10, int start = 0, string? txtSearch = null)
        {
            try
            {
                ...
                var __res = await conn.QueryMultipleAsync("ip_dnreassign_list", __param, commandType: CommandType.StoredProcedure, commandTimeout:180);
        ...
        // debetnote/getbyId/
        public async Task<DNGetByIdforReassignment> GetDNIdforReassignment(int id)
        {
            try
            {
                DNGetByIdforReassignment __res = null!;
                using (IDbConnection conn = Connection)
                {
                    var __param = new DynamicParameters();
                    __param.Add("@id", id);
                    ...
                    {
                        __res = __obj.Read<DNGetByIdforReassignment>().FirstOrDefault()!;
                        if (__res == default)
                        {
                            return __res = null!;
                        }
