...
{
    public class DNListingOverBudgetRepo : IDNListingOverBudgetRepo
    {
        readonly IConfiguration __config;
        public DNListingOverBudgetRepo(IConfiguration config)
        {
            __config = config;
        }
        ...
        // debetnote/getbyId/
        public async Task<DNGetById> GetDNbyIdforDNListingOverBudget(int id)
        {
            try
            {
        ...
        public async Task<object> GetDNOverBudgetList(string periode, int entity, int distributor, int channel, 
            int account, string userid, bool isdnmanual,
            ...
            
        {
            try
            {
                ...
                var __res = await conn.QueryMultipleAsync("ip_debetnote_list_overbudget", __param, commandType: CommandType.StoredProcedure, commandTimeout:180);
