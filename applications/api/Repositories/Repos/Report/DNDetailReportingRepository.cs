...
{
    public class DNDetailReportingRepository : IDNDetailReportingRepo
    {
        readonly IConfiguration __config;
        public DNDetailReportingRepository(IConfiguration config)
        {
            __config = config;
        }
        ...
        public async Task<object> GetDistributorList(string profile)
        {
            using IDbConnection conn = Connection;
            try
            {

                var __query = @"select sud.distributorId, md.LongDesc distributorLongDesc
                        from tbset_user su
                        inner join tbset_user_distributor sud on su.id = sud.UserId
                        inner join tbmst_distributor md on sud.DistributorId = CAST(md.id AS VARCHAR(36))
                        WHERE su.id=@pProfile";

                return await conn.QueryAsync<object>(__query, new {pProfile = profile });
