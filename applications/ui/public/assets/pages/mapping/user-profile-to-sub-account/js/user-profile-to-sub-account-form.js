...
$(document).ready(function () {
    ...
    validator =  FormValidation.formValidation(document.getElementById('form_mapping_user_profile_to_sub_account'), {
        fields: {
            profileId: {
                validators: {
                    notEmpty: {
                        message: "User ID must be enter"
                    },
                }
            },
            channelId: {
    ...
    blockUI.block();
    disableButtonSave();
    Promise.all([getListUserProfile(), getListChannel()]).then(function () {
        blockUI.release();
        enableButtonSave();
    });
});
...
$('#btn_back').on('click', function() {
    window.location.href = '/mapping/user-profile-to-sub-account';
});
...
const save = (exit, e) => {
    let formData = new FormData($('#form_mapping_user_profile_to_sub_account')[0]);
    let url = '/mapping/user-profile-to-sub-account/save';
    $.ajax({
        url         : url,
        data        : formData,
        type        : 'POST',
...
$('#profileId').on('change', async function () {
...
const getListUserProfile = () => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/mapping/user-profile-to-sub-account/list/user-profile",
            type        : "GET",
            dataType    : 'json',
            async       : true,
...
const getListChannel = () => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/mapping/user-profile-to-sub-account/list/channel",
            type        : "GET",
            dataType    : 'json',
            async       : true,
            success: function(result) {
...
const getListSubChannel = (channelId) => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/mapping/user-profile-to-sub-account/list/sub-channel/channel-id",
            type        : "GET",
            dataType    : 'json',
...
const getListAccount = (subChannelId) => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/mapping/user-profile-to-sub-account/list/account/sub-channel-id",
            type        : "GET",
            dataType    : 'json',
...
const getListSubAccount = (accountId) => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/mapping/user-profile-to-sub-account/list/sub-account/account-id",
            type        : "GET",
            dataType    : 'json',
