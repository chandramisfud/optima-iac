...
$(document).ready(function () {
    ...
    validator =  FormValidation.formValidation(document.getElementById('form_mapping_sub_account_blitz'), {
        fields: {
            channelId: {
                validators: {
                    notEmpty: {
            ...
            sapCode: {
                validators: {
                    notEmpty: {
                        message: "Blitz Sub Account Code must be enter"
                    },
                }
            },
        },
    ...
    blockUI.block();
    disableButtonSave();
    Promise.all([getListChannel()]).then(function () {
        blockUI.release();
        enableButtonSave();
    });
});
...
$('#btn_back').on('click', function() {
    window.location.href = '/mapping/sub-account-blitz';
});
...
const save = (exit, e) => {
    let formData = new FormData($('#form_mapping_sub_account_blitz')[0]);
    let url = '/mapping/sub-account-blitz/save';
    $.ajax({
        url         : url,
        data        : formData,
        type        : 'POST',
            ...
            if (!result.error) {
                Swal.fire({
                    ...
                    if (exit) {
                        ...
                        let form = document.querySelectorAll("#form_mapping_sub_account_blitz input[type=text]");
                        formReset(form);
                    }
                });
            } else {
...
const getListChannel = () => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/mapping/sub-account-blitz/list/channel",
            type        : "GET",
            dataType    : 'json',
            async       : true,
...
const getListSubChannel = (channelId) => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/mapping/sub-account-blitz/list/sub-channel/channel-id",
            type        : "GET",
            dataType    : 'json',
...
const getListAccount = (subChannelId) => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/mapping/sub-account-blitz/list/account/sub-channel-id",
            type        : "GET",
            dataType    : 'json',
...
const getListSubAccount = (accountId) => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/mapping/sub-account-blitz/list/sub-account/account-id",
            type        : "GET",
            dataType    : 'json',
