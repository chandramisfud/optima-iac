...
$(document).ready(function () {
    ...
    dt_budget_assignment = $('#dt_budget_assignment').DataTable({
        dom:
            "<'row'<'dt_toolbar'>>" +
            "<'row'<'col-sm-12'Rtr>>" +
            "<'row '<'col-sm-1'l><'col-sm-4'i><'col-sm-7'p>>",
        ajax: {
            url: '/budget/assignment/list/paginate/filter?period=' + $('#filter_period').val(),
            type: 'get',
        },
        processing: true,
        serverSide: true,
                    ...
                    return '\
                        <div class="me-0">\
                            ...
                            <div class="menu menu-sub menu-sub-dropdown w-150px w-md-150px shadow p-3 mb-5 bg-body rounded" data-kt-menu="true">\
                                <a class="dropdown-item text-start edit-record" href="/budget/assignment/form?method=update&id=' + data + '"><i class="fa fa-edit fs-6"></i> Edit Data</a>\
                            </div>\
                        </div>\
                    ';
                }
...
$('#btn_create').on('click', function() {
    checkFormAccess('create_rec', '', '/budget/assignment/form', '')
});

$('#dt_budget_assignment_search').on('keyup', function () {
    dt_budget_assignment.search(this.value).draw();
});
...
$('#dt_budget_assignment_view').on('click', function (){
    let btn = document.getElementById('dt_budget_assignment_view');
    let filter_period = ($('#filter_period').val()) ?? "";
    let filter_entity = ($('#filter_entity').val()) ?? "";
    let filter_distributor = ($('#filter_distributor').val()) ?? "";
    let url = "/budget/assignment/list/paginate/filter?period=" + filter_period + "&entityId=" + filter_entity + "&distributorId=" + filter_distributor;
    btn.setAttribute("data-kt-indicator", "on");
    btn.disabled = !0;
    dt_budget_assignment.ajax.url(url).load(function () {
...
$('#btn_export_excel').on('click', function() {
    ...
    let filter_entity = ($('#filter_entity').val()) ?? "";
    let filter_distributor = ($('#filter_distributor').val()) ?? "";
    let url = "/budget/assignment/export-xls?period=" + filter_period + "&entityId=" + filter_entity + "&distributorId=" + filter_distributor + "&entity=" + text_entity;
