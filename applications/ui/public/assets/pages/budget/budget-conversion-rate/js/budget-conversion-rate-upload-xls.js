...
let dt_list_upload;
let swalTitle = "Budget Conversion Rate Upload";
heightContainer = 430;
...
$('#btn_upload').on('click', function () {
    if ( document.getElementById("file").files.length > 0 ){
        ...
        $.get('/refresh-csrf').done(function(data) {
            ...
            $.ajax({
                url             : '/budget/ss-input/conversion-rate/upload-xls',
                data            : formData,
                type            : 'POST',
                async           : true,
                dataType        : 'JSON',
                ...
                success: async function (result) {
                    if (!result.error) {
                        ...
                        Swal.fire({
                            ...
                            icon: "success",
                            html: 'All failed Budget Conversion Rate Upload items, are shown on the list',
...
const sendEmail = (userApprover, nameApprover, promoId, emailApprover, subject) => {
    return new Promise((resolve, reject) => {
        ...
        $.ajax({
            url             : "/budget/ss-input/conversion-rate/send-email",
            data            : formData,
            type            : 'POST',
            async           : true,
            dataType        : 'JSON',
...
const sendEmailRecon = (userApprover, nameApprover, promoId, emailApprover, subject) => {
    return new Promise((resolve, reject) => {
        ...
        $.ajax({
            url             : "/budget/ss-input/conversion-rate/recon/send-email",
            data            : formData,
            type            : 'POST',
            async           : true,
            dataType        : 'JSON',
