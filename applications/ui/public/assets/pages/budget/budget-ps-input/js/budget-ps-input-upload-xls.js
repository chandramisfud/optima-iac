...
let dt_list_upload;
let swalTitle = "Budget PS Input Upload";
heightContainer = 430;
...
$('#btn_upload').on('click', function () {
    if ( document.getElementById("file").files.length > 0 ){
        ...
        $.get('/refresh-csrf').done(function(data) {
            ...
            $.ajax({
                url             : '/budget/ss-input/ps-input/upload-xls',
                data            : formData,
                type            : 'POST',
                async           : true,
                dataType        : 'JSON',
                ...
                success: async function (result) {
                    if (!result.error) {
                        ...
                        Swal.fire({
                            ...
                            icon: "success",
                            html: 'All failed Budget PS Input Upload items, are shown on the list',
...
const sendEmail = (userApprover, nameApprover, promoId, emailApprover, subject) => {
    return new Promise((resolve, reject) => {
        ...
        $.ajax({
            url             : "/budget/ss-input/ps-input/send-email",
            data            : formData,
            type            : 'POST',
            async           : true,
            dataType        : 'JSON',
...
const sendEmailRecon = (userApprover, nameApprover, promoId, emailApprover, subject) => {
    return new Promise((resolve, reject) => {
        ...
        $.ajax({
            url             : "/budget/ss-input/ps-input/recon/send-email",
            data            : formData,
            type            : 'POST',
            async           : true,
            dataType        : 'JSON',
