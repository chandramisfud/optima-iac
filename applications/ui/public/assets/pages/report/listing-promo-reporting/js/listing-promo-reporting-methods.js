...
let dt_listing_promo_reporting;
let swalTitle = "Listing Promo Reporting";
heightContainer = 315;
...
$(document).ready(function () {
    ...
    if (localStorage.getItem('listingPromoReportingState')) {
        dataFilter = JSON.parse(localStorage.getItem('listingPromoReportingState'));

        url_datatable = '/rpt/listing-promo-reporting/list/paginate/filter?period=' + dataFilter.period +
        ...
        url_datatable = '/rpt/listing-promo-reporting/list/paginate/filter?period=' + $('#filter_period').val() + "&startFrom=" + $('#filter_activity_start').val()  + "&startTo=" + $('#filter_activity_end').val();
    }
    ...
    dt_listing_promo_reporting = $('#dt_listing_promo_reporting').DataTable({
        dom:
            "<'row'<'dt_toolbar'>>" +
            "<'row'<'col-sm-12'Rtr>>" +
    ...
    $('#dt_listing_promo_reporting tbody').on('click', 'td.dt-control', function () {
        let tr = $(this).closest('tr');
        let row = dt_listing_promo_reporting.row(tr);
...
$('#dt_listing_promo_reporting_search').on('keyup', function () {
    dt_listing_promo_reporting.search(this.value).draw();
});

$('#dt_listing_promo_reporting_view').on('click', function (){
    let btn = document.getElementById('dt_listing_promo_reporting_view');
    ...
    let url = "/rpt/listing-promo-reporting/list/paginate/filter?period=" + filter_period + "&startFrom=" + filter_activity_start + "&startTo=" + filter_activity_end
        + "&categoryId=" + filter_category + "&entityId=" + filter_entity + "&distributorId=" + filter_distributor + "&channelId=" + filter_channel;
    btn.setAttribute("data-kt-indicator", "on");
    btn.disabled = !0;
    dt_listing_promo_reporting.ajax.url(url).load(function () {
        ...
        localStorage.setItem('listingPromoReportingState', JSON.stringify(data_filter));
...
$('#btn_export_excel').on('click', function() {
    let text_category = "";
    let data_category = $('#filter_category').select2('data');
    ...
    let filter_distributor = ($('#filter_distributor').val()) ?? "";
    let filter_channel = ($('#filter_channel').val()) ?? "";
    let url = "/rpt/listing-promo-reporting/export-xls?period=" + filter_period + "&startFrom=" + filter_activity_start + "&startTo=" + filter_activity_end + "&categoryId=" + filter_category
...
$('#btn_export_csv').on('click', function() {
    ...
    let filter_distributor = ($('#filter_distributor').val()) ?? "";
    let filter_channel = ($('#filter_channel').val()) ?? "";
    let url = "/rpt/listing-promo-reporting/export-csv?period=" + filter_period + "&startFrom=" + filter_activity_start + "&startTo=" + filter_activity_end + "&categoryId=" + filter_category
        + "&entityId=" + filter_entity + "&distributorId=" + filter_distributor + "&channelId=" + filter_channel + "&category=" + text_category + "&entity=" + text_entity + "&distributor=" + text_distributor;
    let a = document.createElement("a");
    a.href = url;
    let evt = document.createEvent("MouseEvents");
...
const getListDistributor = (entityId) => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/rpt/listing-promo-reporting/list/distributor/entity-id",
            type        : "GET",
            dataType    : 'json',
...
const getListChannel = () => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/rpt/listing-promo-reporting/list/channel",
            type        : "GET",
            dataType    : 'json',
            async       : true,
            success: function(result) {
...
const format = (d) => {
    return '<table class="table table-sm table-condensed" style="margin-left:20px;">' +
        ...
        '</tr></thead>' +
        '<tr>' +
        '<td>' + d.mechanism + '</td><td>' + d.lastStatus + '</td><td>' + formatDate(d.lastStatusDate) + '</td>' +
        '</tr>' +
        '<tr><td></td></tr>' +
        '</table>' +
