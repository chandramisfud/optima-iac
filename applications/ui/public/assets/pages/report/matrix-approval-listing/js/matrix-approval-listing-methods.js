...
$(document).ready(function () {
    ...
    if (localStorage.getItem('matrixApprovalListingReportState')) {
        dataFilter = JSON.parse(localStorage.getItem('matrixApprovalListingReportState'));

        url_datatable = "/rpt/matrix-approval-listing/list/paginate/filter?categoryId=" + (dataFilter.categoryId ?? "0") + "&entityId=" + dataFilter.entityId + "&distributorId=" + dataFilter.distributorId;
    } else {
        url_datatable = "/rpt/matrix-approval-listing/list/paginate/filter?categoryId=" + $('#filter_category').val() + "&entityId=" + $('#filter_entity').val() + "&distributorId=" + $('#filter_distributor').val();
    }
...
$('#dt_matrix_approval_listing_view').on('click', function (){
    ...
    let url = "/rpt/matrix-approval-listing/list/paginate/filter?categoryId=" + filter_category + "&entityId=" + filter_entity + "&distributorId=" + filter_distributor;
    ...
    dt_matrix_approval_listing.ajax.url(url).load(function () {
        ...
        localStorage.setItem('matrixApprovalListingReportState', JSON.stringify(data_filter));
...
$('#btn_export_current').on('click', function() {
    ...
    let filter_entity = ($('#filter_entity').val()) ?? "";
    let filter_distributor = ($('#filter_distributor').val()) ?? "";
    let url = "/rpt/matrix-approval-listing/export-xls?categoryId=" + filter_category + "&entityId=" + filter_entity + "&distributorId=" + filter_distributor + "&entity=" + text_entity;
...
$('#btn_export_historical').on('click', function() {
    ...
    let url = "/rpt/matrix-approval-listing/export-xls/historical?categoryId=" + filter_category + "&entityId=" + filter_entity + "&distributorId=" + filter_distributor + "&entity=" + text_entity;
    let a = document.createElement("a");
    a.href = url;
    let evt = document.createEvent("MouseEvents");
...
const getListCategory = () => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/rpt/matrix-approval-listing/list/category",
            type        : "GET",
            dataType    : 'json',
            async       : true,
...
const getListEntity = () => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/rpt/matrix-approval-listing/list/entity",
            type        : "GET",
            dataType    : 'json',
            async       : true,
            success: function(result) {
...
const getListDistributor = (entityId) => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/rpt/matrix-approval-listing/list/distributor/entity-id",
            type        : "GET",
            dataType    : 'json',
