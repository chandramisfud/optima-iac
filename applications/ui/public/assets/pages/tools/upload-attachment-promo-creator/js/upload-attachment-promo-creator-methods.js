...
$(document).ready(function () {
    ...
    mydropzone = new Dropzone("#dropzone_promo_creator", {
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        type: 'POST',
        url: "/tools/upload-attachment-promo-creator/temp", // Set the url for your upload script location
        paramName: "file",
        maxFilesize: 10, // MB
        addRemoveLinks: true,
        init: function() {
                        ...
                        swal.fire({
                            ...
                            if (result.value) {
                                ...
                                $.ajax({
                                    ...
                                    type: 'POST',
                                    url: '/tools/upload-attachment-promo-creator/temp-delete',
...
$('#btn_download').on('click', function() {
    let url = "/tools/upload-attachment-promo-creator/download-template";
...
$('#btn_upload').on('click', function() {
    ...
    let url = '/tools/upload-attachment-promo-creator/importData';
    $.ajax({
        url: url,
        type: 'POST',
        async: true,
        ...
        success: function (result, status, xhr, $form) {
            ...
            if (!result.error) {
                Swal.fire({
                    ...
                    Dropzone.forElement("#dropzone_promo_creator").removeAllFiles(true);
                });
            } else {
                Swal.fire({
                    title: swalTitle,
...
$('#btn_upload_send').on('click', function () {
    ...
    let url = '/tools/upload-attachment-promo-creator/import';
    e.setAttribute("data-kt-indicator", "on");
    e.disabled = !0;
    blockUI.block();
    $.ajax({
...
const readExcel = (e) => {
    $.ajax({
        url: "/tools/upload-attachment-promo-creator/read-excel",
        type: "GET",
        dataType: "JSON",
        success: async function (result) {
            if (!result.error) {
                ...
                for (let i=1; i <= data.length; i++) {
                    ...
                    await process(dataRow['promoNumber'], dataRow['attachment1'], dataRow['attachment2'], dataRow['attachment3'], dataRow['attachment4']
                        , dataRow['attachment5'], dataRow['attachment6'], dataRow['attachment7']);
...
const process = async (p_promo_number, p_attachment1, p_attachment2, p_attachment3, p_attachment4, p_attachment5, p_attachment6, p_attachment7 ) => {
    return new Promise(function (resolve, reject) {
        ...
        $.ajax({
            type        : 'POST',
            url         : "/tools/upload-attachment-promo-creator/process",
            data        : formData,
            dataType    : "JSON",
            async       : true,
            cache       : false,
