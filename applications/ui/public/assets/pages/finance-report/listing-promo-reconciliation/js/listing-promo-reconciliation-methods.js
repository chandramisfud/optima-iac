...
$(document).ready(function () {
    ...
    if (localStorage.getItem('FinanceListingPromoReconciliationReportingState')) {
        dataFilter = JSON.parse(localStorage.getItem('FinanceListingPromoReconciliationReportingState'));

        url_datatable = '/fin-rpt/listing-promo-recon-reporting/list/paginate/filter?period=' + dataFilter.period +
            "&startFrom=" + dataFilter.activityStart + "&startTo=" + dataFilter.activityEnd +
            "&entityId=" + dataFilter.entityId + "&distributorId=" + dataFilter.distributorId + "&profileId=" + getFilterCreator();

    } else {
        url_datatable = '/fin-rpt/listing-promo-recon-reporting/list/paginate/filter?period=' + $('#filter_period').val() +
            "&startFrom=" + $('#filter_activity_start').val()  + "&startTo=" + $('#filter_activity_end').val() +
            "&entityId=" + $('#filter_entity').val() + "&distributorId=" + $('#filter_distributor').val() + "&profileId=" + getFilterCreator();
    ...
    Promise.all([getListEntity(), getListChannel(), getDataPromoCreator()]).then(async function () {
        if (dataFilter) {
...
$('#dt_listing_promo_reconciliation_view').on('click', function (){
    ...
    let url = "/fin-rpt/listing-promo-recon-reporting/list/paginate/filter?period=" + filter_period + "&startFrom=" + filter_activity_start + "&startTo=" + filter_activity_end
        + "&entityId=" + filter_entity + "&distributorId=" + filter_distributor + "&channelId=" + filter_channel + "&profileId=" + getFilterCreator();
    ...
    dt_listing_promo_reconciliation.ajax.url(url).load(function () {
        ...
        localStorage.setItem('FinanceListingPromoReconciliationReportingState', JSON.stringify(data_filter));
...
$('#btn_export_excel').on('click', function() {
    ...
    let filter_entity = ($('#filter_entity').val()) ?? "";
    let filter_distributor = ($('#filter_distributor').val()) ?? "";
    let url = "/fin-rpt/listing-promo-recon-reporting/export-xls?period=" + filter_period + "&startFrom=" + filter_activity_start + "&startTo=" + filter_activity_end
...
const getDataPromoCreator = () => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/fin-rpt/listing-promo-recon-reporting/get-data/promo-creator/usergroup-id",
            type        : "GET",
            dataType    : 'json',
            async       : true,
            success: function(result) {
                ...
                for (let j = 0, len = result.data.length; j < len; ++j){
                    data.push({
                        id: result.data[j].id,
                        text: result.data[j].id + " [" + result.data[j].username + "]",
                    });
                }
                $('#filter_creator').select2({
                    placeholder: "Select Promo Creator",
                    width: '100%',
                    data: data,
                });
            },
...
const getListEntity = () => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/fin-rpt/listing-promo-recon-reporting/list/entity",
            type        : "GET",
            dataType    : 'json',
            async       : true,
            success: function(result) {
...
const getListDistributor = (entityId) => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/fin-rpt/listing-promo-recon-reporting/list/distributor/entity-id",
            type        : "GET",
            dataType    : 'json',
...
const getListChannel = () => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/fin-rpt/listing-promo-recon-reporting/list/channel",
            type        : "GET",
            dataType    : 'json',
            async       : true,
            success: function(result) {
...
const getFilterCreator = () => {
    //Set Promo Creator (filter_creator)
    let usersCreator = $("#filter_creator").select2().val();
    let users_list = [];
    if (usersCreator.length === 0 || usersCreator[0] === ""){
        users_list.push("ALL")
    } else {
        for (var i = 0; i < usersCreator.length; i++) {
            users_list.push(usersCreator[i]);
        }
    }

    let profileId = encodeURIComponent(JSON.stringify(users_list));
