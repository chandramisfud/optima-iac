...
let dt_listing_promo_reporting_post_recon;
let swalTitle = "Listing Promo Reporting Post Recon";
heightContainer = 315;
...
$(document).ready(function () {
    ...
    if (localStorage.getItem('financeListingPromoReportingPostReconState')) {
        dataFilter = JSON.parse(localStorage.getItem('financeListingPromoReportingPostReconState'));

        url_datatable = '/fin-rpt/listing-promo-reporting-post-recon/list/paginate/filter?period=' + dataFilter.period +
        ...
        url_datatable = '/fin-rpt/listing-promo-reporting-post-recon/list/paginate/filter?period=' + elFilterPeriod.val() + "&startFrom=" + $('#filter_activity_start').val()  + "&startTo=" + $('#filter_activity_end').val();
        url_datatable = '/fin-rpt/listing-promo-reporting-post-recon/list/paginate/filter?period=' + elFilterPeriod.val() + "&startFrom=" + $('#filter_activity_start').val()  + "&startTo=" + $('#filter_activity_end').val();
    }
    ...
    dt_listing_promo_reporting_post_recon = $('#dt_listing_promo_reporting_post_recon').DataTable({
        dom:
            "<'row'<'dt_toolbar'>>" +
            "<'row'<'col-sm-12'Rtr>>" +
    ...
    $('#dt_listing_promo_reporting_post_recon tbody').on('click', 'td.dt-control', function () {
        let tr = $(this).closest('tr');
        let row = dt_listing_promo_reporting_post_recon.row(tr);
...
$('#dt_listing_promo_reporting_post_recon_search').on('keyup', function () {
    dt_listing_promo_reporting_post_recon.search(this.value).draw();
});
...
$('#dt_listing_promo_reporting_post_recon_view').on('click', function (){
    let btn = document.getElementById('dt_listing_promo_reporting_post_recon_view');
    ...
    btn.disabled = !0;
    dt_listing_promo_reporting_post_recon.ajax.url(url).load(function () {
        ...
        localStorage.setItem('financeListingPromoReportingPostReconState', JSON.stringify(data_filter));
    }).on('xhr.dt', function () {
        btn.setAttribute("data-kt-indicator", "off");
        btn.disabled = !1;
    });
...
$('#btn_export_excel').on('click', function() {
    ...
    let filter_channel = ($('#filter_channel').val()) ?? "";
    let url = "/fin-rpt/listing-promo-reporting-post-recon/export-xls?period=" + filter_period + "&startFrom=" + filter_activity_start + "&startTo=" + filter_activity_end + "&categoryId=" + filter_category
...
$('#btn_export_csv').on('click', function() {
    ...
    let filter_distributor = ($('#filter_distributor').val()) ?? "";
    let filter_channel = ($('#filter_channel').val()) ?? "";
    let url = "/fin-rpt/listing-promo-reporting-post-recon/export-csv?period=" + filter_period + "&startFrom=" + filter_activity_start + "&startTo=" + filter_activity_end + "&categoryId=" + filter_category
        + "&entityId=" + filter_entity + "&distributorId=" + filter_distributor + "&channelId=" + filter_channel + "&category=" + text_category + "&entity=" + text_entity + "&distributor=" + text_distributor;
...
const getListCategory = () => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/fin-rpt/listing-promo-reporting-post-recon/list/category",
            type        : "GET",
            dataType    : 'json',
            async       : true,
...
const getListEntity = () => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/fin-rpt/listing-promo-reporting/list/entity",
            type        : "GET",
            dataType    : 'json',
            async       : true,
            success: function(result) {
...
const getListDistributor = (entityId) => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/fin-rpt/listing-promo-reporting-post-recon/list/distributor",
            type        : "GET",
            dataType    : 'json',
...
const getListChannel = () => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/fin-rpt/listing-promo-reporting-post-recon/list/channel",
            type        : "GET",
            dataType    : 'json',
            async       : true,
            success: function(result) {
