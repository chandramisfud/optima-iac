...
$(document).ready(function () {
    ...
    dt_tt_control = $('#dt_tt_control').DataTable({
        dom:
            "<'row'<'dt_toolbar'>>" +
            "<'row'<'col-sm-12'Rtr>>" +
    ...
    $('#dt_tt_control_search').on('keyup', function () {
        dt_tt_control.search(this.value, false, false).draw();
    });
});

$('#dt_tt_control_view').on('click', function (){
    let btn = document.getElementById('dt_tt_control_view');
    ...
    btn.disabled = !0;
    dt_tt_control.ajax.url(url).load(function () {
