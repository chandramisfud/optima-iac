...
$('.btn_download').on('click', function() {
    ...
    $.ajax({
        url         : "/references/sales/check-file",
        type        : "GET",
        data        : {row: row},
        dataType    : 'json',
        async       : true,
...
const checkFile = (row) => {
    return new Promise((resolve, reject) => {
        $.ajax({
            ...
            success: function (result) {
                if (!result.error) {
                    let fileName =  result.files[0].replace('assets/media/references/802/' + row + "/", '');
...
const uploadFile = (file, row) => {
    ...
    let url = '/references/sales/upload';
    $.ajax({
        url: url,
        data: formData,
        type: 'POST',
