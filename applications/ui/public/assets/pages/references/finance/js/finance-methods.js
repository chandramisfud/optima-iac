...
var swalTitle = "Reference Finance";
...
$('.btn_download').on('click', function() {
    ...
    $.ajax({
        url         : "/references/finance/check-file",
        type        : "GET",
        data        : {row: row},
        dataType    : 'json',
        async       : true,
...
const checkFile = (row) => {
    return new Promise((resolve, reject) => {
        $.ajax({
            ...
            success: function (result) {
                if (!result.error) {
                    let fileName =  result.files[0].replace('assets/media/references/801/' + row + "/", '');
...
const uploadFile = (file, row) => {
    ...
    let url = '/references/finance/upload';
    $.ajax({
        url: url,
        data: formData,
        type: 'POST',
        ...
        success: function(result) {
            if (result.error) {
                ...
                Swal.fire({
                    ...
                    $('.review_file_label').addClass('cursor-default form-control-solid-bg').css('border-radius', '.375rem');
                });
            }
        },
        complete: function() {
