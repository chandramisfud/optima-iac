...
var id, arrTaxLevel = [], year, entityId, accountId, promoId, internalOrderNumber;
...
$('#btn_search_promo').on('click',  function () {
    ...
    let url = "/dn/listing-over-budget/get-data/promo?period=" +  year + "&accountId=" + accountId + "&entityId=" + entityId + "&channelId=0";
...
$('#btn_save').on('click', function () {
    ...
    $.get('/refresh-csrf').done(function(data) {
        ...
        let url = '/dn/listing-over-budget/save';
            ...
            success: function(result, status, xhr, $form) {
                if (!result.error) {
                    Swal.fire({
                        ...
                        window.location.href = '/dn/listing-over-budget';
                    });
                } else {
                    Swal.fire({
                        title: swalTitle,
...
const getData = (id) => {
    return new Promise(function (resolve, reject) {
        $.ajax({
            url: "/dn/listing-over-budget/get-data/id",
            type: "GET",
            data: {id: id},
            dataType: 'json',
            async: true,
