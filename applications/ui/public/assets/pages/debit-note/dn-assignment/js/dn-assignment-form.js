...
var swalTitle = "Debit Note Manual [Assignment]";
var id, validator, arrTaxLevel = [], year, entityId, accountId, promoId, internalOrderNumber, usergroupid, approverUserId=0;
...
$(document).ready(function () {
    ...
    Promise.all([getListTaxLevel()]).then(async () => {
        await getData(id);
        await getListSubAccount();
        $('#subAccountId').val(accountId).trigger('change');
        blockUI.release();
    });
});
...
$('#btn_search_promo').on('click',  function () {
    ...
    let url = "/dn/assignment/get-data/promo?period=" +  year + "&subAccountId=" + accountId + "&entityId=" + entityId + "&channelId=0";
...
$('#btn-forward').on('click', function () {
    if (usergroupid === "106") {
        if ($('#internalOrderNumber').val() === '' || $('#internalOrderNumber').val() === undefined) {
            Swal.fire({
                title: swalTitle,
                text: 'Please Enter a internal order number. ',
                icon: "warning",
                buttonsStyling: !1,
...
const forward = () => {
    let e = document.querySelector("#btn-forward");
    let formData = new FormData();
    formData.append('dnId', id);
    formData.append('internalOrderNumber', $('#internalOrderNumber').val());
            ...
            success: function(result, status, xhr, $form) {
                if (!result.error) {
                    Swal.fire({
                        ...
                        window.location.href = '/dn/assignment';
                    });
                } else {
                    Swal.fire({
                        title: swalTitle,
    ...
    return new Promise(function (resolve, reject) {
        $.ajax({
            ...
            success: function (result) {
                if (!result.error) {
                    if (result.usergroupid === '106') {
                        $('#internal-order-number').removeClass('d-none');
                    }
                    ...
                    approverUserId = value.approver_userid;
                    $('#forwardApprover').text(' Forward to ' + value.approver_userid);
                    let total = parseFloat(value.dpp) + parseFloat(value.ppnAmt) - parseFloat(value.pphAmt);
...
const getListTaxLevel = () => {
    return new Promise(function (resolve, reject) {
        $.ajax({
            url         : "/dn/creation/list/tax-level",
            type        : "GET",
            dataType    : 'json',
            async       : true,
            success: function(result) {
