...
var id, arrTaxLevel = [], year, entityId, accountId, promoId, refId;
...
$('#btn_search_promo').on('click',  function () {
    ...
    let url = "/dn/reassignment/get-data/promo?period=" +  year + "&accountId=" + accountId + "&entityId=" + entityId + "&channelId=0";
            ...
            success: function(result, status, xhr, $form) {
                if (!result.error) {
                    Swal.fire({
                        ...
                        window.location.href = '/dn/reassignment';
                    });
                } else {
                    Swal.fire({
                        title: swalTitle,
...
$('#download_all').on('click', function () {
    let url = "/dn/reassignment/download-zip?id=" + id;
...
const getData = (id) => {
    return new Promise(function (resolve, reject) {
        $.ajax({
            url: "/dn/reassignment/get-data/id",
            type: "GET",
            data: {id: id},
            dataType: 'json',
            async: true,
