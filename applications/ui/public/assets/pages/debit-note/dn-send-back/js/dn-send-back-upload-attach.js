    ...
    swal.fire({
        ...
        if(result.value){
            if (data !== ''){
                ...
                $.ajax({
                    url         : '/dn/send-back/delete-attachment',
                    data        : formData,
                    type        : 'POST',
                    async       : true,
                    dataType    : 'JSON',
...
$('#btn_save').on('click', function () {
    ...
    let url;
    if (method) {
        url = '/dn/send-back/update';
        formData.append('id', id);
        formData.append('mode', 'uploadattach');
    } else {
        url = '/dn/send-back/save';
    }
        ...
        success: function(result, status, xhr, $form) {
            if (!result.error) {
                Swal.fire({
                    ...
                    window.location.href = '/dn/send-back';
                });
            } else {
...
const getListTaxLevel = () => {
    return new Promise(function (resolve, reject) {
        $.ajax({
            url         : "/dn/send-back/list/tax-level",
            type        : "GET",
            dataType    : 'json',
            async       : true,
            success: function(result) {
