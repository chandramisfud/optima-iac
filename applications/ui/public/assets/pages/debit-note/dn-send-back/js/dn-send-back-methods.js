...
var dt_dn_creation;
var swalTitle = "Debit Note Send Back";
heightContainer = 280;
let dialerObject;
...
$(document).ready(function () {
    ...
    dt_dn_creation = $('#dt_dn_creation').DataTable({
        ...
        order: [[1, 'asc']],
        ajax: {
            url: "/dn/send-back/list/?period=" +  $('#filter_period').val() + "&subAccountId=" + $('#filter_subaccount').val(),
            type: 'get',
        },
        processing: true,
        serverSide: true,
        ...
        columnDefs: [
            {
                ...
                render: function (data, type, full, meta) {
                    return '<div class="me-0">' +
                        ...
                        '<a class="dropdown-item text-start print-record" href="/dn/send-back/print-pdf?&id=' + data + '" target="_blank"><i class="fa fa-print fs-6 me-2"></i> Print DN</a>' +
                        '<a class="dropdown-item text-start upload-attach" href="/dn/send-back/dn-upload-attach?method=upload&id=' + data + '"><i class="fa fa-cloud-upload-alt fs-6 me-2"></i> Upload DN Attach </a>' +
                        '<a class="dropdown-item text-start view-record" href="/dn/send-back/dn-display?method=view&id=' + data + '"><i class="la la-opencart fs-6 me-2"></i> DN Display </a>' +
                        '</div>' +
                        '</div>';
                }
            },
...
$('#dt_dn_creation_view').on('click', function (){
    ...
    let url = "/dn/send-back/list?period=" + filter_period + "&subAccountId=" + filter_subaccount;
...
const getListSubAccount = () => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/dn/send-back/list/subaccount",
            type        : "GET",
            dataType    : 'json',
            async       : true,
