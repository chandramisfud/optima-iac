...
$('#dt_promo_list').on( 'dblclick', 'tr', async function () {
    let data = dt_promo_list.row( this ).data();

    let elEntity = $('#entityId');

    $('#subAccountId').val(data.subAccountId).trigger('change');

    entityId = data.entityId;
    promoId = data.promoId;

    $('#promoRefId').val(data.refId).trigger('change');
    elEntity.val(data.entityId).trigger('change');
    elEntity.attr("readonly", "readonly");
    ...
    let period = startPromo + ' to ' + endPromo;
    $('#periode').val(period.toString());
...
$('#dt_promo_list_view').on('click',  function () {
    ...
    let url;
    if (method === 'update') {
        url = "/dn/creation-ho/get-data/promo?period=" +  filter_period + "&subAccountId=" + $('#subAccountId').val() + "&entityId=" + $('#entityId').val() + "&channelId=" + filter_channel;
    } else {
        url = "/dn/creation-ho/get-data/promo?period=" +  filter_period + "&subAccountId=0" + "&entityId=" + filter_entity + "&channelId=" + filter_channel;
    }
...
const getListFilterChannel = () => {
    $.ajax({
        url         : "/dn/creation-ho/list/channel",
        type        : "GET",
        dataType    : 'json',
        async       : true,
        success: function(result) {
