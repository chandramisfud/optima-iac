...
let dt_promo_multi_print;
let swalTitle = "Promo Multi Print";
heightContainer = 280;
...
$(document).ready(function () {
    ...
    dt_promo_multi_print = $('#dt_promo_multi_print').DataTable({
        dom:
            "<'row'<'dt_toolbar'>>" +
            "<'row'<'col-sm-12'Rtr>>" +
        ...
        order: [[1, 'asc']],
        ajax: {
            url:'/dn/multi-print-promo/list?period=' + $('#filter_period').val() + "&entityId=" + $('#filter_entity').val(),
            type: 'get',
        },
        processing: true,
        serverSide: false,
...
$('#dt_promo_multi_print_search').on('keyup', function () {
    dt_promo_multi_print.search(this.value).draw();
});
...
$('#dt_promo_multi_print_view').on('click', function (){
    ...
    let url = "/dn/multi-print-promo/list?period=" + filter_period + "&entityId=" + filter_entity ;
...
$('#btn_print').on('click', function () {
    if (arrChecked.length > 0) {
        let url = "/dn/multi-print-promo/print-pdf?id=" + encodeURIComponent(JSON.stringify(arrChecked));
...
const getListEntity = () => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/dn/multi-print-promo/list/entity",
            type        : "GET",
            dataType    : 'json',
            async       : true,
