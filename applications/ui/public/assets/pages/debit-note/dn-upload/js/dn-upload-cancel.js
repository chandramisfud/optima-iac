...
var swalTitle = "Debet Note [cancel]";
...
$(document).ready(function () {
    ...
    validator = FormValidation.formValidation(document.getElementById('form_cancel'), {
        fields: {
            notes: {
                validators: {
                    notEmpty: {
...
$('#btn_cancel').on('click', function () {
    ...
    validator.validate().then(function (status) {
        if (status == "Valid") {
            let e = document.querySelector("#btn_cancel");
            let formData = new FormData($('#form_cancel')[0]);
            formData.append('dnid', id);

            let url = '/dn/upload/cancel';
            $.ajax({
                url         : url,
                data        : formData,
                type        : 'POST',
    ...
    return new Promise(function (resolve, cancel) {
        $.ajax({
            ...
            error: function (jqXHR, textStatus, errorThrown) {
                ...
                return cancel(jqXHR.responseText);
            },
        });
    }).catch((e) => {
        console.log(e);
