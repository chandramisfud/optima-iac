...
let elPeriod = $('#period');
let id, dialerObject, method, validator, arrTaxLevel = [], listEntity = [];
let dnId=0, promoId=0, isCancel, isFreeze, dnRefId, startPromo, endPromo, entityId, refId, taxLevelId, dnWhtType, whtType = "";
...
$(document).ready(function () {
    ...
    if (method === 'update') {
        ...
        Promise.all([getListWHTType(), getListSubAccount(), getListSellingPoint(), getListEntity()]).then(async () => {
            ...
            $('#taxLevel').val(taxLevelId).trigger('change');

            if (isCancel || isFreeze) {
                $('.btn_delete').attr('disabled', true);
                $('#btn_search_promo').attr('disabled', true);
                $('#download_all').attr('disabled', true);
                let txtReason = ''
                if (isCancel) txtReason = ', because this debit note is cancel';
                if (isFreeze) txtReason = ', because this debit note is freeze';
                Swal.fire({
                    title: swalTitle,
                    text: 'Debit Note ' + dnRefId + ' can not edit' + txtReason,
                    icon: "warning",
                    buttonsStyling: !1,
                    ...
                    window.location.href = '/dn/upload';
                });
            }
            enableButtonSave();
            blockUI.release();
...
$('#btn_search_promo').on('click',  function () {
    ...
    let url = "/dn/upload/get-data/promo?period=" +  filter_period + "&subAccountId=" + account_id + "&entityId=" + filter_entity + "&channelId=0";
...
$('#subAccountId').on('change', function () {
    ...
    elPromoRefId.val('').trigger('change');
    $('#periode').val('');
    promoId=0;
    if(method !== 'update'){
...
$('#download_all').on('click', function () {
    let url = "/dn/upload/download-zip?id=" + id;
    ...
    swal.fire({
        ...
        if(result.value){
            if (data !== ''){
                ...
                $.ajax({
                    url         : '/dn/upload/delete-attachment',
                    data        : formData,
                    type        : 'POST',
                    async       : true,
                    dataType    : 'JSON',
        ...
        $.ajax({
            ...
            success: async function (result) {
                if (!result.error) {
                    ...
                    $('#statusPPN').val(value['statusPPN']).trigger('change');
...
const getDataWHTTypePromo = () => {
    return new Promise(function (resolve, reject) {
        $.ajax({
            url: "/dn/upload/get-data/wht-type/promo-id",
            type: "GET",
...
const getListSellingPoint = () => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/dn/upload/list/sellingpoint",
            type        : "GET",
            dataType    : 'json',
            async       : true,
            success: function(result) {
...
const getDataTaxLevel = (entityId, whtType) => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/dn/upload/get-data/tax-level/entity-id",
            type        : "GET",
...
const matchTaxLevel = (str) => {
    arrTaxLevel.forEach(function(el) {
        if (el.taxLevel + " - " + el.description === str) {
            taxLevelId = el.taxLevel;
            return $('#taxLevel').val(el.taxLevel).trigger('change');
        }
    });
}
