        ...
        Promise.all([getListWHTType(), getListSubAccount(), getListSellingPoint(), getListEntity()]).then(async () => {
            ...
            if (isCancel || isFreeze) {
                ...
                Swal.fire({
                    ...
                    window.location.href = '/dn/creation';
                });
            }
            enableButtonSave();
            blockUI.release();
...
$('#btn_search_promo').on('click',  function () {
    ...
    let url = "/dn/creation/get-data/promo?period=" +  filter_period + "&subAccountId=" + account_id + "&entityId=" + filter_entity + "&channelId=0";
...
$('#btn_save').on('click', function () {
    validator.validate().then(function (status) {
        if (status === "Valid") {
            ...
            let url;
            if (method) {
                url = '/dn/creation/update';
                formData.append('id', id);
                formData.append('taxLevelId', taxLevelId);
            } else {
                url = '/dn/creation/save';
            }
    ...
    swal.fire({
        ...
        if(result.value){
            if (data !== ''){
                ...
                $.ajax({
                    url         : '/dn/creation/delete-attachment',
                    data        : formData,
                    type        : 'POST',
                    async       : true,
                    dataType    : 'JSON',
...
const getDataWHTTypePromo = () => {
    return new Promise(function (resolve, reject) {
        $.ajax({
            url: "/dn/creation/get-data/wht-type/promo-id",
            type: "GET",
...
const getListSellingPoint = () => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/dn/creation/list/sellingpoint",
            type        : "GET",
            dataType    : 'json',
            async       : true,
            success: function(result) {
