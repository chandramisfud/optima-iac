...
var swalTitle = "Invoice [Reject]";
var id, idn, method, validator, arrTaxLevel = [];
...
$(document).ready(function () {
    ...
    idn = url_str.searchParams.get("idn");
                ...
                success: function(result, status, xhr, $form) {
                    if (!result.error) {
                        Swal.fire({
                            ...
                            if(method === 'update') {
                                window.location.href = '/dn/invoice/form?method=update&id=' + idn;
                            } else {
                                window.location.href = '/dn/invoice/form';
                            }
                        });
                    } else {
                        Swal.fire({
...
const getData = (id) => {
    return new Promise(function (resolve, reject) {
        $.ajax({
            url: "/dn/send-to-danone/get-data/id",
            type: "GET",
            data: {id: id},
            dataType: 'json',
            async: true,
...
const getListTaxLevel = () => {
    return new Promise(function (resolve, reject) {
        $.ajax({
            url         : "/dn/send-to-danone/list/tax-level",
            type        : "GET",
            dataType    : 'json',
            async       : true,
            success: function(result) {
