...
let elDtMechanismListCreation = $('#dt_mechanism_list_creation');
let dt_mechanism, dt_mechanism_list, dt_mechanism_list_creation, dt_sku, statusMechanismList, yearPromo;
...
$(document).ready(function () {
    ...
    Inputmask({
        ...
        autoGroup: true,
        groupSeparator: ",",
    }).mask("#remainingBudget, #totalCost, #baseline, #totalSales, #cost, #baselineBefore, #totalSalesBefore, #costBefore");
    ...
    Inputmask({
        ...
        groupSeparator: ",",
        suffix: ' %'
    }).mask("#uplift, #salesContribution, #storesCoverage, #redemptionRate, #roi, #cr, #upliftBefore, #salesContributionBefore, #storesCoverageBefore, #redemptionRateBefore, #roiBefore, #crBefore");
    ...
    dt_mechanism_list_creation = elDtMechanismListCreation.DataTable({
        dom:
            "<'row'<'dt_toolbar'>>" +
            "<'row'<'col-sm-12'Rtr>>",
        ordering: false,
        ...
        createdRow: function (row, data, dataIndex) {
            ...
            $('td:eq(2)', row).attr('colspan', '4');
            $('.deleted_cell', row).remove();
            dt_mechanism_list_creation.rows().every(function (rowIdx, tableLoop, rowLoop) {
                if (dataIndex === rowIdx) {
                    let incrSales = (Math.round(data['baseline']) * (data['uplift'] / 100));
                    let roi = Math.round(((incrSales - data['cost']) / data['cost']) * 100);
                    if (!isFinite(roi)) {
...
$('#btn_export_pdf').on('click', function() {
    window.open( "/promo/display/export-pdf?id=" + promoId +"&statusMechanismList=" + statusMechanismList + '&sy=' + yearPromo, "_blank");
});
        ...
        $.ajax({
            ...
            success: async function (result) {
                if (!result['error']) {
                    ...
                    $('#totalCost').val((promo['cost'] ? formatMoney(promo['cost'],0) : 0));
                    ...
                    if (promo['mechanismInputMethod']) {
                        ...
                        $('#mechanismFreeTextCreation').removeClass('d-none');
