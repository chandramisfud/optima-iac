    ...
    return new Promise((resolve, reject) => {
        ...
        if (method === 'update') {
            ...
            let data = {
                ...
                subActivityTypeId: $('#subActivityTypeId').val(),
        ...
        $.ajax({
            ...
            success: async function (result) {
                if (!result['error']) {
                    ...
                    strActivityName = promo['activityDesc'];
...
const saveData = async (modifyReason) => {
    ...
    if (!isPromoExist) {
        if (method !== "update") {
            ...
            let dtNow = new Date().getTime();
            let diffDays = Math.ceil((dtStart - dtNow) / (1000 * 60 * 60 * 24));
            fvPromo.addField('initiatorNotes', {
                validators: {
                    notEmpty: {
            ...
            if (diffDays < latePromoDays) {
                swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        fvPromo.revalidateField('initiatorNotes');
                ...
                fvPromo.removeField('initiatorNotes');
...
const submitSave = async function (modifyReason) {
    ...
    let url = "/promo/creation/revamp/save-dc";
    if (method === "update") {
        url = "/promo/creation/revamp/update-dc";
        formData.append('promoId', promoId);
    }
