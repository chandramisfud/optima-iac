...
let swalTitleAttribute = "Promo Creation - Attribute";
let mechanism_row;
...
$('#btn_attribute_submit').on('click', async function () {
    ...
    if (newMechanismMethod) {
        ...
        if (dataBrand.length<=0) {
            err_message = "Please tick sub brand at least one";
            validateAttribute = 6;
        }
...
const getMechanism = () => {
    return new Promise((resolve, reject) => {
        ...
        $.ajax({
            url         : "/promo/creation/list/mechanism",
            type        : "GET",
            data        : {
                entityId: entityId,
