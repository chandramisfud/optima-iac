...
let targetPromoCalculatorCreation = document.querySelector(".card_promo_calculator_before");
let blockUIPromoCalculatorCreation = new KTBlockUI(targetPromoCalculatorCreation, {
    message: '<div class="blockui-message"><span class="spinner-border text-optima"></span> Loading...</div>',
});
//</editor-fold>
...
$(document).ready(function () {
    ...
    blockUIPromoCalculatorCreation.block();
    disableButtonSave();
    getListAttribute().then(async function () {
        ...
        blockUIPromoCalculatorCreation.release();
        enableButtonSave();
    });
});
//</editor-fold>
...
const roiFormulaBefore = () => {
    ...
    let elTotalSales = $('#totalSalesBefore');
    let elRoi = $('#roiBefore');
...
const getData = (pId) => {
    return new Promise((resolve) => {
        $.ajax({
            url: "/promo/creation/dc-recon/id",
            type: "GET",
            data: {id: pId},
            dataType: "JSON",
            success: async function (result) {
                if (!result['error']) {
                    ...
                    $('#baselineBefore').val('0');
                    $('#upliftBefore').val((promo['upLiftBfr'] === 0 ? '' : promo['upLiftBfr'] ?? 0));
                    $('#totalSalesBefore').val((promo['totalSalesBfr'] === 0 ? '' : promo['totalSalesBfr'] ?? 0));
                    $('#salesContributionBefore').val((promo['salesContributionBfr'] === 0 ? '' : promo['salesContributionBfr'] ?? 0));
                    $('#storesCoverageBefore').val((promo['storesCoverageBfr'] === 0 ? '' : promo['storesCoverageBfr'] ?? 0));
                    $('#redemptionRateBefore').val((promo['redemptionRateBfr'] === 0 ? '' : promo['redemptionRateBfr'] ?? 0));
                    $('#crBefore').val((promo['crBfr'] === 0 ? '' : promo['crBfr'] ?? 0));
                    $('#costBefore').val((promo['costBfr'] === 0 ? '' : promo['costBfr'] ?? 0));
...
const submitSave = async function (modifyReason) {
    ...
    let url = "/promo/creation/recon/update-dc";
    formData.append('promoId', promoId);
