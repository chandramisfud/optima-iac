...
let dt_promo_approval_recon, arrChecked = [];
let swalTitle = "Promo Approval";
let elDtPromoApproval = $('#dt_promo_approval_recon');
heightContainer = 290;
...
$(document).ready(function () {
    ...
    dt_promo_approval_recon = $('#dt_promo_approval_recon').DataTable({
        dom:
            "<'row'<'dt_toolbar'>>" +
            "<'row'<'col-sm-12'Rtr>>" +
            "<'row'<'col-sm-12'>>" +
                ...
                render: function (data, type, full, meta) {
                    ...
                    return `<div class="me-0">
                                ...
                                <div class="menu menu-sub menu-sub-dropdown w-175px w-md-175px shadow p-3 mb-5 bg-body rounded" data-kt-menu="true">
                                    <a class="dropdown-item text-start approve-record" href="/promo/approval-recon/form-approve?id=${full['promoId']}&c=${full['categoryShortDesc']}&sy=${startYear}"><i class="fa fa-edit fs-6"></i> Select Approval Action</a>
                                </div>
                        </div>`;
                }
            },
        ...
        initComplete: function( settings, json ) {
            $('#dt_promo_approval_recon_wrapper').on('change', 'thead th #dt-checkbox-header', function () {
                let data = dt_promo_approval_recon.rows( { filter : 'applied'} ).data();
                arrChecked = [];
                if (this.checked) {
    ...
    elDtPromoApproval.on('change', 'tbody td .dt-checkboxes', function () {
        let rows = dt_promo_approval_recon.row(this.closest('tr')).data();
        if (this.checked) {
            arrChecked.push({
...
$('#dt_promo_approval_recon_search').on('keyup', function() {
    dt_promo_approval_recon.search(this.value, false, false).draw();
});

$('#dt_promo_approval_recon_view').on('click', function (){
    let btn = document.getElementById('dt_promo_approval_recon_view');
    let filter_category = ($('#filter_category').val()) ?? "";
    let filter_entity = ($('#filter_entity').val()) ?? "";
    let filter_distributor = ($('#filter_distributor').val()) ?? "";
    let url = "/promo/approval-recon/list?entityId=" + filter_entity + "&distributorId=" + filter_distributor + "&categoryId=" + filter_category;
    btn.setAttribute("data-kt-indicator", "on");
    btn.disabled = !0;
    dt_promo_approval_recon.ajax.url(url).load(function () {
            ...
            if (!res_method.error) {
                ...
                if (i === arrChecked.length) {
                    ...
                    Swal.fire({
                        ...
                        dt_promo_approval_recon.ajax.reload();
                        resetForm();
                    });
                }
            }
...
const approve = (promoId, categoryShortDescEnc) =>  {
...
const getListCategory = () => {
    $.ajax({
        url         : "/promo/approval-recon/list/category",
        type        : "GET",
        dataType    : 'json',
        async       : true,
        success: function(result) {
...
const getListEntity = () => {
    $.ajax({
        url         : "/promo/approval-recon/list/entity",
        type        : "GET",
        dataType    : 'json',
        async       : true,
        success: function(result) {
...
const getListDistributor = (entityId) => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/promo/approval-recon/list/distributor",
            type        : "GET",
            dataType    : 'json',
