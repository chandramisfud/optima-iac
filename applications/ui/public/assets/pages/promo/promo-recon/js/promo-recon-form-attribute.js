...
let mechanism_row, newMechanismMethod;
...
(function(window, document, $) {
    ...
    dt_region = $('#dt_region').DataTable({
        ...
        drawCallback: function (settings, json) {
            if (configRegion != undefined) {
                $('#dt_region_wrapper input[type=checkbox]').attr('disabled', configRegion);
                $('#dt_region input[type=checkbox]').attr('disabled', configRegion);
            }
        },
    });
    ...
    dt_brand = $('#dt_brand').DataTable({
        ...
        drawCallback: function( settings, json ) {
            if (configBrand != undefined) {
                $('#dt_brand_wrapper input[type=checkbox]').attr('disabled', configBrand);
                $('#dt_brand input[type=checkbox]').attr('disabled', configBrand);
            }
        },
    });
    ...
    dt_sku = $('#dt_sku').DataTable({
        ...
        drawCallback: function( settings, json ) {
            if (configSku != undefined) {
                $('#dt_sku_wrapper input[type=checkbox]').attr('disabled', configSku);
                $('#dt_sku input[type=checkbox]').attr('disabled', configSku);
            }
        },
    });
...
$('#btn_attribute_submit').on('click', async function () {
    ...
    if (newMechanismMethod ) {
        ...
        if(!$('#mechanisme1').val()){
            err_message = "Please fill in mechanism";
            validateAttribut = 5;
        }
...
const readMechanismManual = function() { //set mechanisme manual dari form main ke modal
...
const writeMechanismManual = function() { //set mechanisme manual dari modal ke table mechanism di form main
...
const getMechanism = () => {
    return new Promise((resolve, reject) => {
        ...
        $.ajax({
            url         : "/promo/recon/list/mechanism",
            type        : "GET",
            data        : {
                entityId: entityId,
