...
$(document).ready(function () {
    ...
    dt_promo_send_back = $('#dt_promo_send_back').DataTable({
        dom:
            "<'row'<'dt_toolbar'>>" +
            "<'row'<'col-sm-12'Rtr>>" +
            "<'row '<'col-sm-1'l><'col-sm-4'i><'col-sm-7'p>>",
        ajax: {
            url: '/promo/send-back/list?period=' + $('#filter_period').val(),
            type: 'get',
        },
        processing: true,
        serverSide: false,
                    ...
                    return `
                        <div class="me-0">
                            ...
                            <div class="menu menu-sub menu-sub-dropdown w-auto shadow p-3 mb-5 bg-body rounded" data-kt-menu="true">
                                <a class="dropdown-item text-start edit-record" href="/promo/send-back/form?method=update&id=${full['promoId']}&c=${full['categoryShortDesc']}&old=${(full['isOldPromo'] ? 1 : 0)}&recon=${(full['approvalCycle'] === 1 ? 0 : 1)}"><i class="fa fa-edit fs-6"></i> Edit Data</a>
                            </div>
                        </div>
                    `;
                }
...
$('#dt_promo_send_back_search').on('keyup', function () {
    dt_promo_send_back.search(this.value).draw();
});

$('#dt_promo_send_back_view').on('click', function (){
    let btn = document.getElementById('dt_promo_send_back_view');
    ...
    let url = "/promo/send-back/list?period=" + filter_period + "&entityId=" + filter_entity + "&distributorId=" + filter_distributor + "&categoryId=" + filter_category;
    btn.setAttribute("data-kt-indicator", "on");
    btn.disabled = !0;
    dt_promo_send_back.ajax.url(url).load(function () {
...
$('#btn_export_excel').on('click', function() {
    ...
    (dataCategory[0].id !== "") ? text_category = dataCategory[0].text : text_category ='ALL';
    let text_entity = "";
    ...
    let filter_entity = ($('#filter_entity').val()) ?? "";
    let filter_distributor = ($('#filter_distributor').val()) ?? "";
    let url = "/promo/send-back/export-xls?period=" + filter_period + "&entityId=" + filter_entity + "&distributorId=" + filter_distributor + "&categoryId=" + filter_category + "&entity=" + text_entity + "&distributor=" + text_distributor + "&category=" + text_category;
...
const getListCategory = () => {
    $.ajax({
        url         : "/promo/send-back/list/category",
        type        : "GET",
        dataType    : 'json',
        async       : true,
...
const getListEntity = () => {
    $.ajax({
        url         : "/promo/send-back/list/entity",
        type        : "GET",
        dataType    : 'json',
        async       : true,
        success: function(result) {
