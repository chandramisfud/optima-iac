...
let uuid, url_str, validator, method, newMechanismMethod, dt_mechanism, promoId;
let entityId, entityShortDesc, categoryShortDesc, categoryShortDescEnc, distributorId, channelId, subChannelId, accountId, subAccountId, subCategoryId, activityId, subActivityId, createOn, investmentTypeId;
let statusApprovalCode, isCancel, isClose, isCancelLocked, tsCoding, promoRefId, promoPlanningId, allocationId, categoryId;
            ...
            success: function (result) {
                if (!result.error) {
                    if(result.errCode === 200){
                        ...
                        // Set #totalSales ,#totalInvestment, #roi, #costRatio
                        calcPromo();
                        ...
                        for (let i = 0; i < values.mechanism.length; i++) {
                            ...
                            dMechanism["mechanism"] = values.mechanism[i].mechanism;
                            dMechanism["notes"] = values.mechanism[i].notes;
                            dMechanism["productId"] = values.mechanism[i].productId;
                            dMechanism["product"] = values.mechanism[i].product;
                            dMechanism["brandId"] = values.mechanism[i].brandId;
                            dMechanism["brand"] = values.mechanism[i].brand;
                            dMechanism["mechanismId"] = values.mechanism[i].mechanismId;
                            let mechanism = {};
                            mechanism.id = values.mechanism[i].mechanismId;
                            mechanism.mechanism = values.mechanism[i].mechanism;
                            mechanism.notes = values.mechanism[i].notes;
                            mechanism.productId = values.mechanism[i].productId;
                            mechanism.product = values.mechanism[i].product;
                            mechanism.brandId = values.mechanism[i].brandId;
                            mechanism.brand = values.mechanism[i].brand;
...
const getChannel = () => {
    return new Promise((resolve, reject) => {
        let url = "/promo/send-back/list/edit/channel";
        $.ajax({
            url         : url,
            type        : "GET",
...
const getSubChannel = (channelId) => {
    return new Promise((resolve, reject) => {
        let url = "/promo/send-back/list/edit/sub-channel";
        let listChannel = [];
...
const getAccount = (subChannelId) => {
    return new Promise((resolve, reject) => {
        let url = "/promo/send-back/list/edit/account";
        let listSubChannel = [];
...
const getSubAccount = (accountId) => {
    return new Promise((resolve, reject) => {
        let url = "/promo/send-back/list/edit/sub-account";
        let listAccount = [];
...
const getSubActivity = (activityId) => {
    return new Promise((resolve, reject) => {
        let isDeleted = ((createOn < convertStringToDate(appCutOffHierarchy)) ? "all" : "0");
        $.ajax({
            url         : "/promo/send-back/list/sub-activity",
            type        : "GET",
...
const getInvestmentType = (subActivityId) => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/promo/send-back/investment-type",
            type        : "GET",
...
const getConfigRoiCR = (subActivityId) => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url         : "/promo/send-back/config-roi-cr",
            type        : "GET",
            data        : {subActivityId: subActivityId},
            dataType    : 'json',
            async       : true,
...
const calcPromo = () => {
    ...
    let roi = 0;
    if (totalSales !== 0) {
        cr = totalInvestment / totalSales * 100;
    }
    if (investment !== 0) {
        roi = (incrementSales - investment) / investment * 100;
    }
