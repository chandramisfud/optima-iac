...
var dt_activitytype;
var swalTitle = "Master Sub Activity Type";
...
$(document).ready(function () {
    ...
    dt_activitytype = $('#dt_activitytype').DataTable({
        dom:
            "<'row'<'dt_toolbar'>>" +
            "<'row'<'col-sm-12'Rtr>>" +
        ...
        order: [[1, 'asc']],
        ajax: {
            url: '/master/sub-activity-type/list/paginate/filter',
            type: 'get',
        },
        processing: true,
        serverSide: true,
                    ...
                    return '\
                        <div class="me-0">\
                            ...
                            <div class="menu menu-sub menu-sub-dropdown w-125px w-md-125px shadow p-3 mb-5 bg-body rounded" data-kt-menu="true">\
                                <a class="dropdown-item text-start edit-record" href="/master/sub-activity-type/form?method=update&activitytypeid=' + data + '"><i class="fa fa-edit fs-6"></i> Edit Data</a>\
    ...
    $('#dt_activitytype_Search').on('keyup', function () {
        dt_activitytype.search(this.value).draw();
    });
...
$('#btn_create').on('click', function() {
    checkFormAccess('create_rec', '', '/master/sub-activity-type/form', '')
});

$("#dt_activitytype").on('click', '.delete-record', function () {
    let tr = this.closest("tr");
    let trdata = dt_activitytype.row(tr).data();
...
const fDeleteRecord = (id) => {
    ...
    console.log(id);
    $.ajax({
        url: '/master/sub-activity-type/delete',
        data: formData,
        type: 'POST',
        async: true,
        dataType: 'JSON',
...
$('#btn_export_excel').on('click', function() {
    ...
    a.href = "/master/sub-activity-type/export-xls";
