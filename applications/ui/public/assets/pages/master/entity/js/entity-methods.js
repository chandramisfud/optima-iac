...
$(document).ready(function () {
    ...
    dt_entity = $('#dt_entity').DataTable({
        ...
        order: [[1, 'asc']],
        ajax: {
            url: '/master/entity/list/paginate/filter',
            type: 'get',
        },
        processing: true,
        serverSide: true,
                    ...
                    return '\
                        <div class="me-0">\
                            ...
                            <div class="menu menu-sub menu-sub-dropdown w-125px w-md-125px shadow p-3 mb-5 bg-body rounded" data-kt-menu="true">\
                                <a class="dropdown-item text-start edit-record" href="/master/entity/form?method=update&entityid=' + full.id + '"><i class="fa fa-edit fs-6"></i> Edit Data</a>\
    ...
    $('#dt_entity_Search').on('keyup', function () {
        dt_entity.search(this.value).draw();
    });
...
$('#btn_create').on('click', function() {
    checkFormAccess('create_rec', '', '/master/entity/form', '')
});

$("#dt_entity").on('click', '.delete-record', function () {
    let tr = this.closest("tr");
    let trdata = dt_entity.row(tr).data();
    checkFormAccess('delete_rec', trdata.id, '', "Are your sure to delete data " + trdata.refId)
});

const fDeleteRecord = (entityid) => {
    let formData = new FormData();
    formData.append('id', entityid);
    $.ajax({
        url: '/master/entity/delete',
        data: formData,
        type: 'POST',
        async: true,
        dataType: 'JSON',
...
$('#btn_export_excel').on('click', function() {
    ...
    a.href = "/master/entity/export-xls";
