...
var validator, method, entityid, refId;
var swalTitle = "Entity";
...
$(document).ready(function () {
    ...
    entityid = url_str.searchParams.get("entityid");

    if (method === 'update') {
        Promise.all([ blockUI.block(), disableButtonSave() ]).then(async () => {
            await getData(entityid);
            $('#txt_info_method').text('Edit Entity ' + refId);
            enableButtonSave();
            blockUI.release();
        });
    } else {
    ...
    const form = document.getElementById('form_entity');
...
$('#btn_back').on('click', function() {
    window.location.href = '/master/entity';
});
...
const save = (exit, e) => {
    let formData = new FormData($('#form_entity')[0]);
    let url = '/master/entity/save';
    if (method === "update") {
        formData.append('id', entityid);
        url = '/master/entity/update';
    }
                ...
                if (!result.error) {
                    Swal.fire({
                        ...
                        if (exit) {
                            ...
                            let form = document.querySelectorAll("#form_entity input[type=text]");
                            formReset(form);
                        }
                    });
                } else {
...
const getData = (entityid) => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url: "/master/entity/data/id",
            type: "GET",
            data: {id:entityid},
            dataType: "JSON",
