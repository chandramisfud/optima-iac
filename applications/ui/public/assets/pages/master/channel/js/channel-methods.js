...
$(document).ready(function () {
    ...
    dt_channel = $('#dt_channel').DataTable({
        ...
        order: [[1, 'asc']],
        ajax: {
            url: '/master/channel/list/paginate/filter',
            type: 'get',
        },
        processing: true,
        serverSide: true,
                    ...
                    return '\
                        <div class="me-0">\
                            ...
                            <div class="menu menu-sub menu-sub-dropdown w-125px w-md-125px shadow p-3 mb-5 bg-body rounded" data-kt-menu="true">\
                                <a class="dropdown-item text-start edit-record" href="/master/channel/form?method=update&channelid=' + data + '"><i class="fa fa-edit fs-6"></i> Edit Data</a>\
...
$('#btn_create').on('click', function() {
    checkFormAccess('create_rec', '', '/master/channel/form', '')
});

$("#dt_channel").on('click', '.edit-record', function () {
    let tr = this.closest("tr");
    let trdata = dt_channel.row(tr).data();

    let url = '/master/channel/form?method=update&channelid=' + trdata.id;
    checkFormAccess('update_rec', '', url, '')

});

$("#dt_channel").on('click', '.delete-record', function () {
    let tr = this.closest("tr");
    let trdata = dt_channel.row(tr).data();
    checkFormAccess('delete_rec', trdata.id, '', "Are your sure to delete data " + trdata.refId)
});

const fDeleteRecord = (channelid) => {
    let formData = new FormData();
    formData.append('id', channelid);
    $.ajax({
        url: '/master/channel/delete',
        data: formData,
        type: 'POST',
        async: true,
        dataType: 'JSON',
...
$('#btn_export_excel').on('click', function() {
    ...
    a.href = "/master/channel/export-xls";
