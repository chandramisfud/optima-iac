...
let validator;
let swalTitle = "Master Matrix Approval";
...
$('#btn_upload').on('click', function() {
    validator.validate().then(function (status) {
        if (status === "Valid") {
            ...
            let url = '/master/matrix/promoapproval/upload-xls';
            $.ajax({
                url: url,
                data: formData,
                type: 'POST',
                    ...
                    if (!result.error) {
                        Swal.fire({
                            ...
                            if (result['processId'] !== 0) {
                                let handle = window.open('/master/matrix/promoapproval/process-upload?i=' + result['processId']);
                                handle.blur();
                                window.focus();
                            }
                        });
