...
var validator, method, categoryid, refId;
...
$(document).ready(function () {
    ...
    categoryid = url_str.searchParams.get("categoryid");

    if (method === 'update') {
        Promise.all([ blockUI.block(), disableButtonSave() ]).then(async () => {
            await getData(categoryid);
            $('#txt_info_method').text('Edit Category Promo ' + refId);
            enableButtonSave();
            blockUI.release();
        });
    } else {
...
const save = (exit, e) => {
    let formData = new FormData($('#form_category')[0]);
    let url = '/master/category/save';
    if (method === "update") {
        formData.append('id', categoryid);
        url = '/master/category/update';
    }
                ...
                if (!result.error) {
                    Swal.fire({
                        ...
                        if (exit) {
                            ...
                            let form = document.querySelectorAll("#form_category input[type=text]");
                            formReset(form);
                        }
                    });
                } else {
