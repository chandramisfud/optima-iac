...
var validator, method, regionid, refId;
var swalTitle = "Master Region";
...
$(document).ready(function () {
    ...
    regionid = url_str.searchParams.get("regionid");

    if (method === 'update') {
        Promise.all([ blockUI.block(), disableButtonSave() ]).then(async () => {
            await getData(regionid);
            $('#txt_info_method').text('Edit Region ' + refId);
            enableButtonSave();
            blockUI.release();
        });
    } else {
    ...
    const form = document.getElementById('form_region');
...
$('#btn_back').on('click', function() {
    window.location.href = '/master/region';
});
...
const save = (exit, e) => {
    let formData = new FormData($('#form_region')[0]);
    let url = '/master/region/save';
    if (method === "update") {
        formData.append('id', regionid);
        url = '/master/region/update';
    }
                ...
                if (!result.error) {
                    Swal.fire({
                        ...
                        if (exit) {
                            ...
                            let form = document.querySelectorAll("#form_region input[type=text]");
                            formReset(form);
                        }
                    });
                } else {
...
const getData = (regionid) => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url: "/master/region/data/id",
            type: "GET",
            data: {id:regionid},
            dataType: "JSON",
