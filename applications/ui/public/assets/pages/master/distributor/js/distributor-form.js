...
var validator, method, distributorid, refId;
var swalTitle = "Distributor";
...
$(document).ready(function () {
    ...
    distributorid = url_str.searchParams.get("distributorid");

    if (method === 'update') {
        Promise.all([ blockUI.block(), disableButtonSave() ]).then(async () => {
            await getData(distributorid);
            $('#txt_info_method').text('Edit Distributor ' + refId);
            enableButtonSave();
            blockUI.release();
        });
    } else {
    ...
    const form = document.getElementById('form_distributor');
        ...
        fields: {
            ...
            npwp: {
                validators: {
                    stringLength: {
                        max: 255,
                        message: 'Npwp must be less than 255 characters',
                    }
                }
            },
            claimManager: {
                validators: {
                    stringLength: {
                        max: 50,
                        message: 'Klaim Manager must be less than 50 characters',
                    }
                }
            },
        },
...
$('#btn_back').on('click', function() {
    window.location.href = '/master/distributor';
});
...
const save = (exit, e) => {
    let formData = new FormData($('#form_distributor')[0]);
    let url = '/master/distributor/save';
    if (method === "update") {
        formData.append('id', distributorid);
        url = '/master/distributor/update';
    }
                ...
                if (!result.error) {
                    Swal.fire({
                        ...
                        if (exit) {
                            ...
                            let form = document.querySelectorAll("#form_distributor input[type=text]");
                            formReset(form);
                        }
                    });
                } else {
...
const getData = (distributorid) => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url: "/master/distributor/data/id",
            type: "GET",
            ...
            success: function (result) {
                if (!result.error) {
                    ...
                    $('#npwp').val(values.npwp);
                    $('#claimManager').val(values.claimManager);
                    refId = values.refId;
                } else {
                    Swal.fire({
                        title: swalTitle,
