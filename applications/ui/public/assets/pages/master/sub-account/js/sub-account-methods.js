...
var dt_subaccount, subaccountid;
...
$(document).ready(function () {
    ...
    dt_subaccount = $('#dt_subaccount').DataTable({
        ...
        order: [[1, 'asc']],
        ajax: {
            url: '/master/sub-account/list/paginate/filter',
            type: 'get',
        },
        processing: true,
        serverSide: true,
        ...
        columnDefs: [
                    ...
                    return '\
                        <div class="me-0">\
                            ...
                            <div class="menu menu-sub menu-sub-dropdown w-125px w-md-125px shadow p-3 mb-5 bg-body rounded" data-kt-menu="true">\
                                <a class="dropdown-item text-start edit-record" href="/master/sub-account/form?method=update&subaccountid=' + data + '"><i class="fa fa-edit fs-6"></i> Edit Data</a>\
            ...
            },
            {
                targets: 5,
                title: 'Short Desc',
                data: 'subAccountShortDesc',
                className: 'text-nowrap align-middle',
            },
            {
                targets: 6,
                title: 'Long Desc',
                data: 'subAccountLongDesc',
                className: 'text-nowrap align-middle',
            },
        ],
...
$('#btn_create').on('click', function() {
    checkFormAccess('create_rec', '', '/master/sub-account/form', '')
});

$("#dt_subaccount").on('click', '.delete-record', function () {
    let tr = this.closest("tr");
    let trdata = dt_subaccount.row(tr).data();
    checkFormAccess('delete_rec', trdata.subAccountId, '', "Are your sure to delete data " + trdata.subAccountRefId)
});

const fDeleteRecord = (subaccountid) => {
    let formData = new FormData();
    formData.append('id', subaccountid);
    $.ajax({
        url: '/master/sub-account/delete',
        data: formData,
        type: 'POST',
        async: true,
        dataType: 'JSON',
...
$('#btn_export_excel').on('click', function() {
    ...
    a.href = "/master/sub-account/export-xls";
