...
class PromoUploadAttachmentCreator extends Controller
{
    ...
    {
        return view('Tools::upload-attachment-promo-creator.index');
    }
        ...
        try {
            ...
            if ($response->status() === 200) {
                ...
                if ($statusProcess) {
                    ...
                    $apiProcess = config('app.api'). '/tools/upload/promoattachment';
    ...
    protected function sendEmailApproverCycle1($userApprover, $nameApprover, $id, $email, $subject)
    {
        ...
        try {
            ...
            if ($responsePromo->status() === 200) {
                ...
                $message = view('Tools::upload-attachment-promo-creator.new-email-approval', compact('title', 'subtitle', 'data', 'promoId', 'urlid', 'urlrefid' , 'userApprover', 'recon', 'ar_fileattach', 'paramEncrypted'))
                    ->toHtml();
