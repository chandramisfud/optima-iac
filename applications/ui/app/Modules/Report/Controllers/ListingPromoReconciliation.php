...
class ListingPromoReconciliation extends Controller
{
    ...
    public function landingPage()
    {
        ...
        return view('Report::listing-promo-reconciliation.index', compact('title'));
    }

    public function getListPaginateFilter(Request $request)
    {
        $api = config('app.api'). '/report/listingpromorecon';
        try {
            $page = 0;
            if ($request->length > -1) {
    ...
    {
        $api = config('app.api'). '/report/listingpromorecon/entity';
        Log::info('Get API ' . $api);
        try {
    ...
    {
        $api = config('app.api'). '/report/listingpromorecon/distributor';
        $query = [
            'budgetId'  => 0,
            'entityId'  => $request->entityId,
        ];
