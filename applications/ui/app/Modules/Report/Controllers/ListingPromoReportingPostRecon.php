...
class ListingPromoReportingPostRecon extends Controller
{
    ...
    {
        $title = "Listing Promo Reporting Post Recon";
        return view('Report::listing-promo-reporting-post-recon.index', compact('title'));
    }

    public function getListPaginateFilter(Request $request): array|bool|string
    {
        try {
            $api = '/finance-report/listingpromoreportingpostrecon';
    ...
    {
        $api = '/report/listingpromoreporting/category';
        $callApi = new CallApi();
    ...
    {
        $api = '/report/listingpromoreporting/distributor';
        $callApi = new CallApi();
        $query = [
            'entityId'      => $request['entityId'],
        ];
    ...
    {
        $api = '/report/listingpromoreporting/channel';
        $callApi = new CallApi();
        $query = [
            'userid'      => 'admin',
        ];
        ...
        try {
            ...
            if (!json_decode($res)->error) {
                ...
                foreach ($data as $fields) {
                    ...
                    $arr[] = $fields->cost;
                    $arr[] = $fields->baselineBefore;
                    $arr[] = $fields->totalSalesBefore;
                    $arr[] = ($fields->upliftBefore ? $fields->upliftBefore / 100 : 0);
                    $arr[] = ($fields->salesContributionBefore ? $fields->salesContributionBefore / 100 : 0);
                    $arr[] = ($fields->storesCoverageBefore ? $fields->storesCoverageBefore / 100 : 0);
                    $arr[] = ($fields->redemptionRateBefore ? $fields->redemptionRateBefore / 100 : 0);
                    $arr[] = ($fields->crBefore ? $fields->crBefore / 100 : 0);
                    $arr[] = $fields->costBefore;
                ...
                $heading = [
                    ['Listing Promo Reporting Post Recon'],
                    ...
                    [
                        ...
                        'Budget Mass Approval Status', 'Budget Mass Approval Date', 'Budget Mass Approval by', 'Budget Deploy Status', 'Budget Deploy Date', 'Budget Deploy By', 'Smart Code',
                        'Baseline', 'Total Sales', 'Uplift (%)', 'Stores Coverage (%)', 'Sales Contribution (%)', 'Redemption Rate (%)', 'CR (%)', 'Cost',
                        'Baseline Before', 'Total Sales Before', 'Uplift Before (%)', 'Sales Contribution Before (%)', 'Stores Coverage Before (%)', 'Redemption Rate Before (%)', 'CR Before (%)', 'Cost Before',
                        'Main Activity'
                    ]
                ];
                $formatCell =  [
                    ...
                    'BP' => NumberFormat::builtInFormatCode(37),
                    'BQ' => NumberFormat::builtInFormatCode(37),
                    'BR' => NumberFormat::FORMAT_PERCENTAGE_00,
                    'BS' => NumberFormat::FORMAT_PERCENTAGE_00,
                    'BT' => NumberFormat::FORMAT_PERCENTAGE_00,
                    'BU' => NumberFormat::FORMAT_PERCENTAGE_00,
                    'BV' => NumberFormat::FORMAT_PERCENTAGE_00,
                    'BW' => NumberFormat::builtInFormatCode(37),
                ];
                $title = 'A1:BX1'; //Report Title Bold and merge
                $header = 'A6:BX6'; //Header Column Bold and color
                $headerCustom = 'BP6:BW6';
                $filename = 'Listing Promo Reporting Post Recon - ';
                $export = new Export($result, $heading, $title, $header, $formatCell, $headerCustom);
                $mc = microtime(true);
    ...
    public function exportCsv(Request $request): StreamedResponse|bool|array|string
    {
        try {
            ...
            if (!json_decode($res)->error) {
                ...
                $mc = microtime(true);
                $fileName = 'Listing Promo Reporting Post Recon - ' . date('Y-m-d His') . $mc . '.csv';
                ...
                $callback = function () use ($resVal, $columns, $period, $category, $entity) {
                    ...
                    fputcsv($file, ["Listing Promo Reporting Post Recon"], ",");
                    ...
                    foreach ($resVal as $task) {
                        ...
                        $row['CR (%)']                      = $task->cr;
                        $row['Cost']                        = $task->cost;
                        $row['Baseline Before']              = $task->baselineBefore;
                        $row['Total Sales Before']           = $task->totalSalesBefore;
                        $row['Uplift Before (%)']            = $task->upliftBefore;
                        $row['Sales Contribution Before (%)']= $task->salesContributionBefore;
                        $row['Stores Coverage Before (%)']   = $task->storesCoverageBefore;
                        $row['Redemption Rate Before (%)']   = $task->redemptionRateBefore;
                        $row['CR Before (%)']                = $task->crBefore;
                        $row['Cost Before']                  = $task->costBefore;
                        ...
                        fputcsv($file, array(
                            ...
                            $row['Cost'],
                            $row['Baseline Before'],
                            $row['Total Sales Before'],
                            $row['Uplift Before (%)'],
                            $row['Sales Contribution Before (%)'],
                            $row['Stores Coverage Before (%)'],
                            $row['Redemption Rate Before (%)'],
                            $row['CR Before (%)'],
                            $row['Cost Before'],
                            $row['Main Activity']
                        ), ",");
