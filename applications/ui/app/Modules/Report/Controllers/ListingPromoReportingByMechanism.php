...
class ListingPromoReportingByMechanism extends Controller
{
    ...
    public function landingPage(): Factory|View|Application
    {
        ...
        return view('Report::listing-promo-reporting-by-mechanism.index', compact('title'));
    }

    public function getListPaginateFilter(Request $request): bool|string
    {
        $api = '/report/listingpromoreportingbymechanism';
        ...
        try {
            ...
            if (!json_decode($res)->error) {
                ...
                foreach ($data as $fields) {
                    ...
                    $arr[] = (($fields->sendbackNotes=='') ? '' : $fields->sendbackNotes . ' on ' . $fields->sendbackNotesDate);
                    $arr[] = $fields->target;
                    $arr[] = $fields->investment;
                    $arr[] = $fields->investmentTypeRefId;
                    $arr[] = $fields->investmentTypeDesc;
                    $arr[] = (($fields->investmentTypeRefIdPromo=='---' || $fields->investmentTypeRefIdPromo=='' || $fields->investmentTypeRefIdPromo==null)
                        ? '---' : $fields->investmentTypeRefIdPromo. ' - ' . $fields->investmentTypeRefIdPromo);
                    $arr[] = $fields->dnClaim;
                    $arr[] = $fields->dnPaid;
                ...
                $formatCell =  [
                    ...
                    'AZ' => NumberFormat::builtInFormatCode(37),
                    'BA' => NumberFormat::builtInFormatCode(37),
                    'BB' => NumberFormat::FORMAT_PERCENTAGE_00,
                    'BC' => NumberFormat::FORMAT_PERCENTAGE_00,
                    'BD' => NumberFormat::FORMAT_PERCENTAGE_00,
                    'BE' => NumberFormat::FORMAT_PERCENTAGE_00,
                    'BF' => NumberFormat::FORMAT_PERCENTAGE_00,
                    'BG' => NumberFormat::builtInFormatCode(37),
                ];
                $title = 'A1:BH1'; //Report Title Bold and merge
                $header = 'A6:BH6'; //Header Column Bold and color
            ...
            if (!json_decode($res)->error) {
                ...
                $callback = function () use ($resVal, $columns, $period, $category, $entity) {
                    ...
                    foreach ($resVal as $task) {
                        ...
                        $row['Last Update']                 = date('d-m-Y' , strtotime($task->lastUpdate));
                        ...
                        $row['Promo Start']                 = date('d-m-Y' , strtotime($task->startPromo));
                        $row['Promo End']                   = date('d-m-Y' , strtotime($task->endPromo));
                        $row['Creation Date']               = date('d-m-Y' , strtotime($task->createOn));
