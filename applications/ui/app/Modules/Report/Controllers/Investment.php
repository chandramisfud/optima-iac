...
class Investment extends Controller
{
    ...
    public function landingPage()
    {
        ...
        return view('Report::investment.index', compact('title'));
    }

    public function getListPaginateFilter(Request $request)
    {
        $api = config('app.api'). '/report/investment';
        try {
            $page = 0;
            if ($request->length > -1) {
    ...
    {
        $api = config('app.api'). '/report/investment/budgetallocation';
        $query = [
            'year'          => $request->year,
            'entityId'      => 0,
            'distributorId' => 0,
    ...
    {
        $api = config('app.api'). '/report/investment/entity';
        Log::info('Get API ' . $api);
        try {
    ...
    {
        $api = config('app.api'). '/report/investment/distributor';
        $query = [
            'budgetId'  => 0,
            'entityId'  => $request->entityId,
        ];
