...
class MatrixApprovalListing extends Controller
{
    ...
    public function landingPage()
    {
        ...
        return view('Report::matrix-approval-listing.index', compact('title'));
    }

    public function getListPaginateFilter(Request $request)
    {
        $api = config('app.api'). '/report/matrixapprovallisting';
        try {
            $page = 0;
            if ($request->length > -1) {
    ...
    {
        $api = config('app.api'). '/report/matrixapprovallisting/category';
        Log::info('Get API ' . $api);
        try {
    ...
    {
        $api = config('app.api'). '/report/matrixapprovallisting/entity';
        Log::info('Get API ' . $api);
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api);
            if ($response->status() === 200) {
    ...
    {
        $api = config('app.api'). '/report/matrixapprovallisting/distributor';
        $query = [
            'budgetId'  => 0,
            'entityId'  => $request->entityId,
        ];
    ...
    public function exportXlsHistorical(Request $request) {
        $api = config('app.api'). '/report/matrixapprovallisting/history';
        $query = [
