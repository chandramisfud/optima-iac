...
class DocumentCompleteness extends Controller
{
    ...
    {
        $api = config('app.api'). '/report/documentcompleteness';
        try {
            $page = 0;
            if ($request->length > -1) {
    ...
    public function landingPage()
    {
        ...
        return view('Report::document-completeness.index', compact('title'));
    }

    public function getListEntity(Request $request)
    {
        $api = config('app.api'). '/report/documentcompleteness/entity';
        Log::info('Get API ' . $api);
        try {
    ...
    {
        $api = config('app.api'). '/report/documentcompleteness/distributor';
        $query = [
            'budgetId'  => 0,
            'entityId'  => $request->entityId,
        ];
