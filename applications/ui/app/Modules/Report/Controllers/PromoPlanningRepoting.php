...
class PromoPlanningRepoting extends Controller
{
    ...
    public function landingPage()
    {
        ...
        return view('Report::promo-planning-reporting.index', compact('title'));
    }

    public function getListPaginateFilter(Request $request)
    {
        $api = config('app.api'). '/report/promoplanningreporting';
        try {
            $page = 0;
            if ($request->length > -1) {
    ...
    {
        $api = config('app.api'). '/report/promoplanningreporting/entity';
        Log::info('Get API ' . $api);
        try {
    ...
    {
        $api = config('app.api'). '/report/promoplanningreporting/distributor';
        $query = [
            'EntityId'           => $request->entityId,
        ];
        Log::info('Get API ' . $api);
    ...
    {
        $api = config('app.api'). '/report/promoplanningreporting/channel';
        Log::info('Get API ' . $api);
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api);
            if ($response->status() === 200) {
