...
class PromoHistoricalMovement extends Controller
{
    ...
    public function landingPage()
    {
        ...
        return view('FinanceReport::promo-historical-movement.index', compact('title'));
    }

    public function getListPaginateFilter(Request $request)
    {
        $api = config('app.api'). '/finance-report/promohistoricalmovement';
        try {
            $page = 0;
            if ($request->length > -1) {
    ...
    {
        $api = config('app.api'). '/finance-report/promohistoricalmovement/entity';
        Log::info('Get API ' . $api);
        try {
    ...
    {
        $api = config('app.api'). '/finance-report/promohistoricalmovement/distributor';
        $query = [
            'budgetId'  => 0,
            'entityId'  => $request->entityId,
        ];
