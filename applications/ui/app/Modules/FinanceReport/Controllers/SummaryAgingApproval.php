...
class SummaryAgingApproval extends Controller
{
    ...
    public function landingPage()
    {
        ...
        return view('FinanceReport::summary-aging-approval.index', compact('title'));
    }

    public function getListPaginateFilter(Request $request)
    {
        $api = config('app.api'). '/finance-report/summaryagingapproval';
        try {
            $page = 0;
            if ($request->length > -1) {
    ...
    {
        $api = config('app.api'). '/finance-report/summaryagingapproval/entity';
        Log::info('Get API ' . $api);
        try {
    ...
    {
        $api = config('app.api'). '/finance-report/summaryagingapproval/distributor';
        $query = [
            'budgetId'  => 0,
            'entityId'  => $request->entityId,
        ];
