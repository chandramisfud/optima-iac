...
class DNDisplay extends Controller
{
    ...
    {
        $title = "Debit Note";
        return view('FinanceReport::dn-display.index', compact('title'));
    }

    public function getListPaginateFilter(Request $request)
    {
        $api = config('app.api'). '/finance-report/dndisplay';
        try {
            $page = 0;
    ...
    {
        $title = "Debit Note";
        return view('FinanceReport::dn-display.form', compact('title'));
    }

    public function getDataDNById(Request $request)
    {
        $api = config('app.api'). '/finance-report/dndisplay/id';
        $query = [
            'id'  => $request->id,
        ];
        Log::info('Get API ' . $api);
    ...
    {
        $api = config('app.api'). '/finance-report/dndisplay/entity';
        Log::info('Get API ' . $api);
        try {
    ...
    {
        $api = config('app.api'). '/finance-report/dndisplay/distributor';
        $query = [
            'budgetId'  => 0,
            'entityId'  => $request->entityId,
        ];
    ...
    {
        $api = config('app.api'). '/finance-report/dndisplay/sellingpoint';
        Log::info('Get API ' . $api);
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api);
            if ($response->status() === 200) {
    ...
    {
        $api = config('app.api'). '/finance-report/dndisplay/taxlevel';
        Log::info('Get API ' . $api);
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api);
            if ($response->status() === 200) {
