...
class ListingDN extends Controller
{
    ...
    public function landingPage()
    {
        ...
        return view('FinanceReport::listing-dn.index', compact('title'));
    }

    public function getListPaginateFilter(Request $request)
    {
        $api = config('app.api'). '/finance-report/listingdn';
        try {
            $page = 0;
    ...
    {
        $api = config('app.api'). '/finance-report/listingdn/entity';
        Log::info('Get API ' . $api);
        try {
    ...
    {
        $api = config('app.api'). '/finance-report/listingdn/distributor';
        $query = [
            'budgetId'  => 0,
            'entityId'  => $request->entityId,
        ];
