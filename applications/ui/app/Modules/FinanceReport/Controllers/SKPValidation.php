...
class SKPValidation extends Controller
{
    ...
    public function landingPage()
    {
        ...
        return view('FinanceReport::skp-validation.index', compact('title'));
    }

    public function getListPaginateFilter(Request $request)
    {
        $api = config('app.api'). '/finance-report/skpvalidation';
        try {
            $page = 0;
            if ($request->length > -1) {
    ...
    {
        $api = config('app.api'). '/finance-report/skpvalidation/entity';
        Log::info('Get API ' . $api);
        try {
    ...
    {
        $api = config('app.api'). '/finance-report/skpvalidation/distributor';
        $query = [
            'EntityId'           => $request->entityId,
        ];
        Log::info('Get API ' . $api);
    ...
    {
        $api = config('app.api'). '/finance-report/skpvalidation/channel';
        $query = [
            'userid'           => 0,
        ];
        Log::info('Get API ' . $api);
