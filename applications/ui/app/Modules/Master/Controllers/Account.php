...
class Account extends Controller
{
    ...
    public function getListPaginateFilter(Request $request) {
        $api = config('app.api'). '/master/account';
        try {
            $page = 0;
            if ($request->length > -1) {
    ...
    public function accountFormPage(Request $request) {
        return view('Master::account.form');
    }

    public function getListChannel(Request $request) {
        $api = config('app.api'). '/master/account/channel';
        try {
    ...
    public function getDataSubchannel(Request $request) {
        $api = config('app.api'). '/master/account/subchannel';
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api, [
    ...
    public function getDataByID(Request $request) {
        $api = config('app.api'). '/master/account/id';
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api, [
                'id'  => $request->id
            ]);
