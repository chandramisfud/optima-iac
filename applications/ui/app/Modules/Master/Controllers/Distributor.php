...
use App\Exports\Master\ExportViewDistributor;
...
class Distributor extends Controller
{
    ...
    {
        return view('Master::distributor.index');
    }

    public function getListPaginateFilter(Request $request) {
        $api = config('app.api'). '/master/distributor';
        try {
            $page = 0;
            if ($request->length > -1) {
    ...
    public function distributorFormPage(Request $request) {
        return view('Master::distributor.form');
    }

    public function getDataByID(Request $request) {
        $api = config('app.api'). '/master/distributor/id';
        try {
    ...
    public function save(Request $request) {
        ...
        $data = [
            ...
            "npwp"          => $request->npwp,
            "claimManager"  => $request->claimManager,
        ...
        try {
            ...
            if ($response->status() == 200) {
                ...
                foreach ($resVal as $fields) {
                    $arr = [];
                    $arr[] = $fields->refId;
                    $arr[] = $fields->companyName;
                    $arr[] = $fields->shortDesc;
                    $arr[] = $fields->longDesc;
                    $arr[] = $fields->npwp;
                    $arr[] = $fields->address;
                    $arr[] = $fields->claimManager;

                    $result[] = $arr;
                }

                $filename = 'Master Distributor - ';
                $title = 'A1:G1'; //Report Title Bold and merge
                $header = 'A3:G3'; //Header Column Bold and color
                $heading = [
                    ['Master Distributor'],
                    ['Date Retrieved : ' . date('Y-m-d')],
                    ['Distributor ID', 'Company Name', 'Short Desc', 'Long Desc', 'NPWP', 'Address', 'Claim Manager']
                ];
