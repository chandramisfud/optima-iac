...
use App\Exports\Master\ExportViewMasterSubChannel;
use Maatwebsite\Excel\Facades\Excel;
use PhpOffice\PhpSpreadsheet\Style\NumberFormat;

use Wording;

class SubChannel extends Controller
{
    ...
    {
        return view('Master::sub-channel.index');
    }

    public function getListPaginateFilter(Request $request) {
        $api = config('app.api'). '/master/subchannel';
        try {
            $page = 0;
            if ($request->length > -1) {
    ...
    public function subchannelFormPage(Request $request) {
        return view('Master::sub-channel.form');
    }

    public function getListChannel(Request $request) {
        $api = config('app.api'). '/master/subchannel/channel';
        try {
    ...
    public function getDataByID(Request $request) {
        $api = config('app.api'). '/master/subchannel/id';
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api, [
                'id'  => $request->id
            ]);
