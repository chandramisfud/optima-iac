...
use App\Exports\Master\ExportViewMasterCategory;
...
class Category extends Controller
{
    ...
    public function getListPaginateFilter(Request $request) {
        $api = config('app.api'). '/master/category';
        try {
            $page = 0;
            if ($request->length > -1) {
    ...
    public function categoryFormPage(Request $request) {
        return view('Master::category.form');
    }

    public function getDataByID(Request $request) {
        $api = config('app.api'). '/master/category/id';
        try {
