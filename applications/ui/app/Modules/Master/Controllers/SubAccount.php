...
use App\Exports\Master\ExportViewMasterSubAccount;
...
class SubAccount extends Controller
{
    ...
    {
        return view('Master::sub-account.index');
    }

    public function getListPaginateFilter(Request $request) {
        $api = config('app.api'). '/master/subaccount';
        try {
            $page = 0;
            if ($request->length > -1) {
    ...
    public function subAccountFormPage(Request $request) {
        return view('Master::sub-account.form');
    }

    public function getListChannel(Request $request) {
        $api = config('app.api'). '/master/subaccount/channel';
        try {
    ...
    public function getDataSubchannel(Request $request) {
        $api = config('app.api'). '/master/subaccount/subchannel';
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api, [
    ...
    public function getDataAccount(Request $request) {
        $api = config('app.api'). '/master/subaccount/account';
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api, [
    ...
    public function getDataByID(Request $request) {
        $api = config('app.api'). '/master/subaccount/id';
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api, [
                'id'  => $request->id
            ]);
