...
use App\Exports\Master\ExportViewMasterRegion;
...
class Region extends Controller
{
    ...
    public function getListPaginateFilter(Request $request) {
        $api = config('app.api'). '/master/region';
        try {
            $page = 0;
            if ($request->length > -1) {
    ...
    public function regionFormPage(Request $request) {
        return view('Master::region.form');
    }

    public function getDataByID(Request $request) {
        $api = config('app.api'). '/master/region/id';
        try {
