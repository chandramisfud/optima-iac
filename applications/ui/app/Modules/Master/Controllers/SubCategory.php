...
use App\Exports\Master\ExportViewMasterSubCategory;
...
class SubCategory extends Controller
{
    ...
    public function getListPaginateFilter(Request $request) {
        $api = config('app.api'). '/master/subcategory';
        try {
            $page = 0;
            if ($request->length > -1) {
    ...
    public function subCategoryFormPage(Request $request) {
        return view('Master::sub-category.form');
    }

    public function getListCategory(Request $request) {
        $api = config('app.api'). '/master/subcategory/category';
        try {
    ...
    public function getDataByID(Request $request) {
        $api = config('app.api'). '/master/subcategory/id';
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api, [
                'id'  => $request->id
            ]);
        ...
        try {
            ...
            if ($response->status() == 200) {
                ...
                foreach ($resVal as $fields) {
                    ...
                    $arr[] = $fields->subCategoryRefId;
                    $arr[] = $fields->categoryLongDesc;
                    $arr[] = $fields->subCategoryShortDesc;
                    $arr[] = $fields->subCategoryLongDesc;

                    $result[] = $arr;
                }

                $filename = 'Master Sub Category Promo - ';
                $title = 'A1:D1'; //Report Title Bold and merge
                $header = 'A3:D3'; //Header Column Bold and color
                $heading = [
                    ['Master Sub Category Promo'],
                    ['Date Retrieved : ' . date('Y-m-d')],
                    ['Sub Category ID', 'Category', 'Short Desc', 'Long Desc']
                ];
