...
use App\Exports\Master\ExportViewMasterProfitCenter;
use Maatwebsite\Excel\Facades\Excel;
use PhpOffice\PhpSpreadsheet\Style\NumberFormat;

use Wording;

class ProfitCenter extends Controller
{
    ...
    {
        return view('Master::profit-center.index');
    }

    public function getListPaginateFilter(Request $request) {
        $api = config('app.api'). '/master/profitcenter';
        try {
            $page = 0;
            if ($request->length > -1) {
    ...
    public function profitCenterFormPage(Request $request) {
        return view('Master::profit-center.form');
    }

    public function getDataByID(Request $request) {
        $api = config('app.api'). '/master/profitcenter/id';
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api, [
                'ProfitCenter'  => $request->id
            ]);
            if ($response->status() === 200) {
    ...
    public function save(Request $request) {
        ...
        $data = [
            ...
            "profitCenterDesc"  => $request->description,
        ];
