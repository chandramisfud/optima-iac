...
use App\Exports\Master\ExportViewMasterSku;
...
class Sku extends Controller
{
    ...
    {
        return view('Master::sku.index');
    }

    public function getListPaginateFilter(Request $request) {
        $api = config('app.api'). '/master/product';
        try {
            $page = 0;
            if ($request->length > -1) {
    ...
    public function skuFormPage(Request $request) {
        return view('Master::sku.form');
    }

    public function getListEntity(Request $request) {
        $api = config('app.api'). '/master/product/entity';
        try {
    ...
    public function getDataBrand(Request $request) {
        $api = config('app.api'). '/master/product/brand';
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api, [
    ...
    public function getDataByID(Request $request) {
        $api = config('app.api'). '/master/product/id';
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api, [
                'id'  => $request->id
            ]);
    ...
    public function delete(Request $request): false|array|string
    {
        $api = config('app.api') . '/master/product';
        $data = [
            'id'            => $request['id']
        ...
        try {
            ...
            if ($response->status() == 200) {
                ...
                foreach ($resVal as $fields) {
                    ...
                    $arr[] = $fields->productRefId;
                    $arr[] = $fields->entityLongDesc;
                    $arr[] = $fields->brandLongDesc;
                    $arr[] = $fields->productShortDesc;
                ...
                $heading = [
                    ...
                    ['SKU ID', 'Entity', 'Brand', 'Short Desc', 'Long Desc']
                ];
