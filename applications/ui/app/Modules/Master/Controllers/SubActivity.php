...
use App\Exports\Master\ExportViewMasterSubActivity;
use Maatwebsite\Excel\Facades\Excel;
use PhpOffice\PhpSpreadsheet\Style\NumberFormat;

use Wording;

class SubActivity extends Controller
{
    ...
    {
        return view('Master::sub-activity.index');
    }

    public function getListPaginateFilter(Request $request) {
        $api = config('app.api'). '/master/subactivity';
        try {
            $page = 0;
            if ($request->length > -1) {
    ...
    public function subActivityFormPage(Request $request) {
        return view('Master::sub-activity.form');
    }

    public function getListCategory(Request $request) {
        $api = config('app.api'). '/master/subactivity/category';
        try {
    ...
    public function getDataSubCategory(Request $request) {
        $api = config('app.api'). '/master/subactivity/subcategory';
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api, [
    ...
    public function getDataActivity(Request $request) {
        $api = config('app.api'). '/master/subactivity/activity';
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api, [
    ...
    public function getListSubActivityType(Request $request) {
        $api = config('app.api'). '/master/subactivity/activitytype';
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api);
            if ($response->status() === 200) {
                $resVal = json_decode($response)->values;
    ...
    public function getDataByID(Request $request) {
        $api = config('app.api'). '/master/subactivity/id';
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api, [
                'id'  => $request->id
            ]);
    ...
    public function save(Request $request) {
        ...
        $data = [
            ...
            "subActivityTypeId" => $request->activitytype,
