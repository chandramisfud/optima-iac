...
class Brand extends Controller
{
    ...
    public function getListPaginateFilter(Request $request) {
        $api = config('app.api'). '/master/brand';
        try {
            $page = 0;
            if ($request->length > -1) {
    ...
    public function brandFormPage (Request $request) {
        return view('Master::brand.form');
    }

    public function getListEntity(Request $request) {
        $api = config('app.api'). '/master/brand/entity';
        try {
    ...
    public function getDataByID(Request $request) {
        $api = config('app.api'). '/master/brand/id';
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api, [
                'id'  => $request->id
            ]);
