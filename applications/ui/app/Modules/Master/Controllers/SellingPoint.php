...
use App\Exports\Master\ExportViewMasterSellingPoint;
use Maatwebsite\Excel\Facades\Excel;
use PhpOffice\PhpSpreadsheet\Style\NumberFormat;

use Wording;

class SellingPoint extends Controller
{
    ...
    {
        return view('Master::selling-point.index');
    }

    public function getListPaginateFilter(Request $request) {
        $api = config('app.api'). '/master/sellingpoint';
        try {
            $page = 0;
            if ($request->length > -1) {
    ...
    public function sellingpointFormPage(Request $request) {
        return view('Master::selling-point.form');
    }

    public function getListRegion(Request $request) {
        $api = config('app.api'). '/master/sellingpoint/region';
        try {
    ...
    public function getListProfitCenter(Request $request) {
        $api = config('app.api'). '/master/sellingpoint/profitcenter';
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api);
            if ($response->status() === 200) {
                $resVal = json_decode($response)->values;
    ...
    public function getDataByID(Request $request) {
        $api = config('app.api'). '/master/sellingpoint/id';
        try {
            $response = Http::timeout(180)->withToken($this->token)->get($api, [
                'id'  => $request->id
            ]);
        ...
        try {
            ...
            if ($response->status() == 200) {
                ...
                foreach ($resVal as $fields) {
                    ...
                    $arr[] = $fields->sellingPointRefId;
                    $arr[] = $fields->regionLongDesc;
                    $arr[] = $fields->areaCode;
                    $arr[] = $fields->profitCenterDesc;
                    $arr[] = $fields->sellingPointShortDesc;
                    $arr[] = $fields->sellingPointLongDesc;

                    $result[] = $arr;
                }

                $filename = 'Master Selling Point - ';
                $title = 'A1:F1'; //Report Title Bold and merge
                $header = 'A3:F3'; //Header Column Bold and color
                $heading = [
                    ['Master Selling Point'],
                    ['Date Retrieved : ' . date('Y-m-d')],
                    ['Selling Point ID', 'Region', 'Area Code', 'Profit Center', 'Short Desc', 'Long Desc']
                ];
