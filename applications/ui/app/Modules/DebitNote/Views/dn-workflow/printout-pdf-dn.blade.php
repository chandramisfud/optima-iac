...
<div class="lampiran">
    <table class="date_info">
        <tr>
            ...
            $sellPoint = "";
            if (@$data->sellpoint) {
                $listSellPoint = $data->sellpoint;
                foreach ($listSellPoint as $dataSellPoint) {
                    if ($dataSellPoint->flag) {
                        $sellPointCode = $dataSellPoint->sellpoint;
                        $sellPoint = $dataSellPoint->longDesc;
                    }
                }
            }
            @endphp
            <td>(({{ @$sellPointCode . " - " . @$sellPoint }})), {{ date('Y-m-d', strtotime(@$data->deductionDate)) }}</td>
        </tr>
    </table>

    <table class="outlet_info">
        <tr>
            <td style="width: 20%">Nama Outlet</td>
            <td>:</td>
            <td>TERLAMPIR</td>
        </tr>
        <tr>
