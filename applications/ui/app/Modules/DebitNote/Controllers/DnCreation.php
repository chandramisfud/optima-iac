...
class DnCreation extends Controller
{
    ...
    {
        $title = "Debit Note";
        return view('DebitNote::dn-creation.index', compact('title'));
    }
    ...
    {
        $title = "Debit Note";
        return view('DebitNote::dn-creation.form', compact('title'));
    }

    public function dnDisplay()
    {
        $title = "Debit Note";
        return view('DebitNote::dn-creation.dn-display', compact('title'));
    }

    public function uploadAttachPage()
    {
        $title = "Debit Note";
        return view('DebitNote::dn-creation.upload-attach', compact('title'));
    }

    public function cancelPage()
    {
        $title = "Debit Note";
        return view('DebitNote::dn-creation.cancel', compact('title'));
    }
                ...
                if (json_decode($response)->code === 666) {
                    ...
                    if ($request->promoId == 0) {
                        ...
                        if ($responseEmail->status() === 200) {
                            ...
                            $messageEmail = "Update Debet Note ID " . ($values->refId ?? "") . " has been successfuly but can't send email notification to " . ($values->approverUserid ?? "") . " - " . ($values->approverUserName ?? "") . " (" . ($values->approverEmail ?? "").")";
                            $dataResult = array_merge($dataResult, [
                                'error'     => true,
